...
un file di salvataggio e metterlo su un supporto fisico, cioe' una cartuccia tramite il
lettore oppure la scheda SD della console modificata. E' l'unica operazione irreversibile
del progetto, ed e' la sola dove un errore banale, per esempio scrivere un salvataggio da
128 KiB su una memoria da 64, distrugge dati di vent'anni.

Questo strumento e' la parte di quell'operazione che si puo' scrivere e collaudare oggi,
senza avere l'hardware in mano, ed e' anche la parte che evita i danni: valida il file,
identifica il gioco, verifica che i backup obbligatori esistano davvero, e produce il piano
dei passi da eseguire. La scrittura vera non e' implementata e non lo sara' finche' il
lettore non e' presente e collaudato: lo strumento la dichiara e si ferma.

La regola normativa che questo codice mette in atto e' `.claude/rules/hardware-and-perimeter.md`,
cioe' nessuna scrittura senza backup in doppia copia su percorsi distinti e verificato
leggibile, e nessuna scrittura senza rilettura confrontata byte per byte.

Cosa non fa, dichiarato
-----------------------
Non scrive, non apre porte seriali, non parla con FlashGBX ne' con la console. Non modifica
il file di salvataggio in alcun modo: lo apre in sola lettura. Non giudica la legalita' del
contenuto, che e' un'altra cosa e appartiene agli strumenti di verifica come PKHeX.
...
# fuori da questa tabella non e' necessariamente sbagliata, ma e' sempre sospetta, e la
...
DIMENSIONI = {
    ...
    131072 + 16: "flash da 128 KiB piu' un'intestazione di 16 byte aggiunta da alcuni strumenti",
}

# Le destinazioni possibili. La colonna che conta e' l'ultima: cosa manca perche' quella
# destinazione diventi eseguibile. Nessuna e' eseguibile oggi.
DESTINAZIONI = {
    "gbxcart": {
        "descrizione": "cartuccia originale tramite il lettore GBxCart RW, con FlashGBX",
        "dimensioni_ammesse": (32 * 1024, 64 * 1024, 128 * 1024),
        "eseguibile": False,
        "manca": "il lettore, che non e' ancora arrivato; e un collaudo su una cartuccia sacrificabile prima di toccare quella di Smeraldo",
    },
    "3ds-sd": {
        ...
        "eseguibile": False,
        "manca": "la decisione su quale percorso della SD sia quello giusto per il titolo, che dipende da come e' installato; e la conferma che il salvataggio sia nel formato che quel titolo si aspetta",
    },
    "file": {
        "descrizione": "copia su disco, che non e' hardware ma e' la destinazione dei collaudi",
        "dimensioni_ammesse": tuple(DIMENSIONI.keys()),
        "eseguibile": False,
        "manca": "nulla di tecnico: e' disabilitata per coerenza, perche' una copia di file la fa il sistema operativo e non serve uno strumento",
    },
}
...
def carica_diagnostica():
    """Importa il lettore di salvataggi Gen 3 del track Smeraldo, se e' al suo posto.

    La logica di validazione dei settori, di scelta dello slot piu' recente e di
    identificazione del gioco vive la' e non va duplicata qui: un secondo esemplare della
    stessa formula e' un secondo posto dove sbagliarla.
    """
...
def esamina(percorso):
    """Ritorna un dizionario di fatti sul file, senza giudizio, piu' la lista dei problemi."""
    ...
    # La firma di settore e' il primo controllo diagnostico che conta su un salvataggio di
    # terza generazione, e la sua assenza e' il sintomo che su GBAtemp e' costato due giorni
    # a chi non lo conosceva: se non c'e', quel file non e' un salvataggio Gen 3 valido,
    ...
    if firme == 0:
        esito["note"].append(
            "nessun settore porta la firma 0x08012025: non e' un salvataggio di terza generazione. "
            "Se doveva essere un salvataggio Game Boy di prima o seconda generazione, la firma non e' attesa e questa nota non e' un errore"
        )
    else:
        diag = carica_diagnostica()
        if diag is None:
            esito["note"].append("il lettore di diagnosi del track Smeraldo non e' al suo posto, quindi slot e gioco non sono stati identificati")
        else:
            ...
            try:
                ...
                # indovinare: qui interessa il primo, e il margine sul secondo, perche' un
                # margine nullo significa che il gioco non e' identificato.
                ...
                if esito["gioco"] is None:
                    esito["problemi"].append("il gioco non e' identificabile dal contenuto: scrivere un salvataggio senza sapere di che gioco e' non e' un'operazione da fare")
            except Exception as errore:  # la diagnosi e' informativa: il suo fallimento non deve nascondere gli altri esiti
                esito["note"].append("la ricostruzione dello slot non e' riuscita: %s" % errore)

    if all(b == 0 for b in blob):
        esito["problemi"].append("il file e' interamente a zero: non contiene un salvataggio")
    if all(b == 0xFF for b in blob):
        esito["problemi"].append("il file e' interamente a 0xFF, che e' lo stato di una memoria flash cancellata")
...
def verifica_backup(percorsi, dimensione_attesa):
    ...
    if len(set(reali)) < len(reali):
        problemi.append("due percorsi di backup coincidono: una copia sola non e' una doppia copia")
    ...
    for p in reali:
        ...
        try:
            ...
            problemi.append("il backup non e' leggibile, quindi non e' un backup: %s (%s)" % (p, errore))
            continue
        if dimensione_attesa is not None and os.path.getsize(p) != dimensione_attesa:
            problemi.append("il backup %s misura %d byte e l'originale %d: non e' una copia dello stesso supporto" % (p, os.path.getsize(p), dimensione_attesa))
    if len(impronte) > 1:
        problemi.append("i due backup hanno contenuto diverso: uno dei due non e' la copia che si crede")
    return problemi
...
def piano(esito, destinazione, backup):
    ...
    print("Destinazione   %s, cioe' %s" % (nome, dest["descrizione"]))
    ...
    # Il piano non si mostra quando le precondizioni non sono soddisfatte, e la ragione e'
    ...
    if ostacoli:
        ...
        print("Il vincolo del backup in doppia copia su percorsi distinti e verificato leggibile e' normativo, vedi .claude/rules/hardware-and-perimeter.md, e non si aggira per fretta.")
        return 1

    print("Backup         due copie distinte, leggibili e di dimensione coerente: il vincolo normativo e' soddisfatto")
    print("")
    ...
    print("  2. confronto del dump con i backup dichiarati: se differiscono, fermarsi e capire perche' prima di scrivere")
    print("  3. scrittura del file sul supporto")
    print("  4. rilettura del supporto e confronto byte per byte con il file scritto, che e' l'unico modo per dire che la scrittura e' avvenuta")
    print("  5. avvio del gioco e verifica che il salvataggio si carichi, che e' un controllo diverso dal precedente e non lo sostituisce")
    print("")
    if not dest["eseguibile"]:
        print("La scrittura non viene tentata: questa destinazione non e' eseguibile.")
...
def main():
    ...
    c = sub.add_parser("check", help="esamina un file di salvataggio e dice se e' scrivibile")
    c.add_argument("file")
    ...
    p.add_argument("--backup", action="append", default=[], help="percorso di un backup gia' esistente; va ripetuto due volte")
