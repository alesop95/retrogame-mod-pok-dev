...
## 20. Scritto e verificato sulla cartuccia vera, il 2026-09-21

La Grotta Mutevole è uscita dal giro lo stesso 2026-09-21, rimandata per intero: l'utente ha deciso di giocare quelle otto specie per davvero invece di sbloccarle e scriverle, e di riprendere la domanda dentro `pokedex-home-completo`. Il deposito PC è rimasto, con un criterio diverso da quello di questa sezione (fuori ciò che si compra in negozio, dentro ciò che non si compra, dettagli in `STUDIO-01` sezione 22). L'RTC, aperto lo stesso giorno, ne è uscito anch'esso: la pila tampone non è ancora stata sostituita, e scrivere il permesso senza alimentazione nuova sarebbe un lavoro da rifare.

La catena finale, tre strumenti di questo studio più `emerald_pc_storage_fix.py`, è stata eseguita sul dump del 2026-09-21 (793 ore 41 minuti di gioco, non 788: l'utente aveva continuato a giocare, compreso uno scambio con una terza persona che ha anche cambiato la chiave di sicurezza del salvataggio, `STUDIO-01` sezione 24). Quaranta byte di differenza in tutto, confinati alla tasca Oggetti Chiave, al deposito PC, ai flag qui trattati e ai due checksum di sezione coinvolti. Scritto sulla cartuccia vera con FlashGBX, riletto senza disconnettere, hash SHA-256 identico fra il file scritto e il read-back (`bc661a1a…`): la scrittura è verificata in modo indipendente, non solo dichiarata riuscita.

Il controllo visivo in gioco, riferito dallo stesso giorno, è positivo su ogni punto. Al porto di Alghepoli sono ripartite le scene di prima consegna; il Sig. Marino e la Mappa Stinta hanno portato all'Isola Suprema, dove Mew (livello 30) è risultato incontrabile, affrontato senza cattura né salvataggio per sola verifica; al Monte Cordone sia Lugia sia Ho-Oh sono risultati giocabili, entrambi livello 70; all'Isola Materna il triangolo del rompicapo di Deoxys era presente; tornando ad Alghepoli il menu mostrava tutte e sei le voci. La console è stata spenta senza salvare, quindi il file già verificato per read-back resta lo stato esatto della cartuccia. Il fronte delle isole è chiuso in tutti i suoi anelli: diagnosi, scrittura, verifica indipendente, controllo in gioco.
