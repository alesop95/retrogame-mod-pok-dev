...
Delle tre basi di dati una riguarda direttamente questo progetto ed è quella dei doni segreti, perché contiene la medesima tabella da cui `catalogo-eventi-gen3.py` genera il nostro catalogo: centosettantasette voci contro le diciassette del corpus impiegato dal nostro generatore. È la via di produzione in volume, ed è discussa nella sezione 8 di `recreate-pokemon-distributions-events/STUDIO-04`.
