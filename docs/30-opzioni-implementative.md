...
A questa opzione si e' aggiunto un riferimento trovato leggendo una discussione che era rimasta fra le fonti non lette, ed e' un buon argomento contro il lasciarne indietro. L'organizzazione `CableClub` pubblica `cable-link`, letto, che e' il precedente piu' avanzato che il progetto conosca per questa strada.

Contiene un circuito stampato completo in formato KiCad, con schema, sbroglio e file gerber pronti per la produzione, e un firmware per Raspberry Pi Pico che apre la porta seriale a 500 kHz, cioe' esattamente il massimo che Pan Docs dichiara riconosciuto dal Game Boy monocromatico. Il file `src/pokemon_gen1_link_protocol.h` porta la macchina a stati del protocollo di generazione 1 con tutte le sue costanti, e come mostra [[08-cavo-link]] coincidono una per una con quelle che avevamo ricavato dal disassemblato. La licenza e' Apache 2.0 e il lavoro e' del 2021.

Che cosa significa in pratica per una decisione. L'opzione D non parte da zero: parte da un PCB progettato e da un firmware che ha completato scambi fra Game Boy reali, su un microcontrollore da pochi euro e ampiamente documentato. Cio' che resta da scrivere e' la parte che quel firmware non fa, cioe' la conversione, che come dice [[20-architettura-codice]] vive sul PC. Resta vero che serve saper cablare o saldare, e resta vera l'obiezione sullo spirito del progetto di riferimento, ma il costo di sviluppo di questa opzione e' molto piu' basso di quanto sembrasse prima di leggere quel repository.
