...
### 8.8 Il nome, e la catena dei passaggi di una singola estrazione

La sigla LCRNG sta per Linear Congruential Random Number Generator, cioè generatore lineare congruenziale di numeri casuali, e i tre aggettivi vanno letti come una definizione compressa: lineare perché il passo è una funzione affine dello stato, congruenziale perché il risultato si riduce per congruenza modulo un intero fissato, e di numeri casuali per convenzione e non per proprietà, poiché la successione è completamente determinata dal seme e nulla in essa è casuale. La qualificazione corretta è pseudocasuale, e la distinzione non è pedanteria: è precisamente ciò che rende possibile il lavoro di ricostruzione descritto in questa area, perché una successione veramente casuale non si invertirebbe.

La nozione esiste perché il nome non dice come si usa, e il come si usa è una catena di passaggi che va scritta una volta per intero. Chi conosce la formula del passo non sa ancora che cosa il gioco faccia in un'estrazione, e quel che il gioco fa comprende almeno tre operazioni che la formula non nomina.

L'esempio svolto è la catena completa di una singola estrazione nel dominio, nei suoi cinque passaggi. Primo, si legge lo stato corrente, che è una parola di trentadue bit conservata in memoria e non un valore visibile in alcun campo di alcun esemplare. Secondo, si moltiplica quello stato per il moltiplicatore, ottenendo un prodotto che sta in sessantaquattro bit. Terzo, si somma l'incremento. Quarto, si riduce per modulo due alla trentaduesima, che sull'hardware non è una divisione ma il semplice scarto dei bit oltre il trentaduesimo, cioè il troncamento naturale del registro: qui la scelta del modulo come potenza di due si paga zero. Quinto, e questo è il passaggio che la formula non contiene, si scrive il nuovo stato al posto del vecchio e si consegna al chiamante non lo stato ma la sua parola alta, cioè i sedici bit superiori.

Ne segue una distinzione che vale enunciare perché genera confusione ogni volta che non è dichiarata: lo stato del generatore e il valore estratto sono due cose diverse, di larghezza diversa, e un solo passo produce un solo valore estratto consumando un solo stato. Contare le estrazioni consumate da un metodo, che è l'operazione centrale del lavoro sugli eventi, significa contare quante volte questa catena è stata percorsa, non quante parole di trentadue bit sono comparse.

L'esempio si chiude sulla composizione, che è il passaggio successivo e non appartiene al generatore. Il valore di personalità di un esemplare di terza generazione ha trentadue bit e nasce da due estrazioni consecutive: la parola alta della prima diventa la metà bassa e quella della seconda la metà alta, oppure il contrario, e quale delle due disposizioni valga è precisamente la firma del metodo di generazione. I valori individuali nascono da altre due estrazioni, con quindici bit informativi ciascuna. Un esemplare da evento è dunque il risultato di quattro percorsi della catena, e conoscere il generatore senza conoscere la disposizione non basta a riprodurlo.

Si impiega in `recreate-pokemon-distributions-events/STUDIO-02-metodi-di-generazione.md` e in [[06-identita-pokemon]].

### 8.9 La forma chiusa del termine ennesimo

Il passo del generatore si applica ripetutamente, ma non occorre applicarlo per conoscere il termine ennesimo: la successione ha una forma chiusa, ottenuta risolvendo la ricorrenza affine come si risolve una serie geometrica.

```
s(n) = (a^n * s(0) + c * (a^n - 1) / (a - 1)) modulo m
```

La nozione esiste perché separa due domande che si confondono. Una è quanto valga lo stato dopo un numero noto di passi, e la forma chiusa la risolve con un'elevazione a potenza modulare, cioè in un tempo proporzionale al logaritmo del numero di passi. L'altra è quanti passi separino due stati noti, che è il problema inverso e non si risolve con questa formula: richiede un logaritmo discreto, che su modulo potenza di due è trattabile ma non gratuito, ed è la ragione per cui in pratica si procede per bisezione sull'esponente come descritto nella voce 8.5.

La divisione che compare nella formula va letta con cautela, e la cautela è il contenuto tecnico della voce. Il denominatore `a - 1` non è invertibile modulo una potenza di due, perché le condizioni di periodo massimo della voce 8.2 impongono che sia divisibile per quattro e quindi pari. La scrittura come frazione è dunque un abuso di notazione ereditato dal caso reale: il valore corretto è la somma geometrica dei primi `n` termini calcolata modulo `m`, cioè la sommatoria che la voce 8.5 riporta per l'incremento del salto in avanti. Chi implementa la forma chiusa invertendo il denominatore ottiene un risultato sbagliato senza alcun errore visibile, ed è un difetto che si manifesta soltanto come valori che non tornano.

L'esempio svolto è il conto del costo. Per sapere dove sia il generatore dopo un milione di passi, la via diretta costa un milione di iterazioni; la forma chiusa costa una elevazione a potenza modulare, cioè circa venti moltiplicazioni per il quadrato ripetuto, più il calcolo della somma geometrica con la medesima tecnica. Il rapporto è di quattro ordini di grandezza, ed è lo stesso rapporto della voce 8.5 perché è lo stesso meccanismo visto da un'altra parte.

Si impiega in `recreate-pokemon-distributions-events/STUDIO-02-metodi-di-generazione.md`.

### 8.10 Il generatore come sistema a stato, e l'uscita come troncamento

Un generatore lineare congruenziale si descrive utilmente come un sistema dinamico a tempo discreto, autonomo e a stato finito: lo stato è una parola di trentadue bit, la legge di evoluzione è la funzione affine del passo, e non esiste alcun ingresso oltre alla condizione iniziale. L'uscita non coincide con lo stato ma è una sua funzione, cioè la selezione dei sedici bit superiori.

La nozione esiste perché questa descrizione rende immediata una proprietà che altrimenti va argomentata caso per caso, ed è la proprietà che governa tutto il lavoro di ricostruzione: il sistema è deterministico, quindi la traiettoria è funzione della sola condizione iniziale, e l'osservatore che vede l'uscita non vede lo stato. Fra i due c'è una perdita, e la perdita è esattamente di sedici bit per passo.

Ne segue la formulazione corretta del problema di ricostruzione, che è un problema di osservabilità. Osservare un'uscita significa conoscere sedici dei trentadue bit dello stato che l'ha prodotta; osservarne due consecutive significa conoscere trentadue bit distribuiti su due stati legati dalla legge di evoluzione. La domanda se lo stato sia ricostruibile è dunque la domanda se il sistema sia osservabile a partire da quel numero di uscite, e la risposta si ottiene contando i bit informativi come fa la voce 8.7. È la medesima struttura del problema di stima dello stato di un sistema lineare da misure parziali, con la differenza che qui l'aritmetica è modulare e non reale, e che l'assenza di rumore rende la determinazione esatta anziché statistica.

L'esempio svolto è la lettura della restrizione del seme in questi termini. Un metodo che dichiara il seme di origine ristretto a sedici bit non sta scegliendo un numero piccolo per comodità: sta riducendo lo spazio delle condizioni iniziali da due alla trentaduesima a due alla sedicesima, cioè di un fattore sessantacinquemilacinquecentotrentasei. Poiché la ricostruzione consiste nel cercare quale condizione iniziale produca l'uscita osservata, la restrizione è ciò che sposta il problema dall'enumerazione lunga all'enumerazione immediata, come la voce 8.6 calcola. Una restrizione dello spazio degli stati iniziali è, in questo senso, la cosa più utile che una specifica possa dichiarare a chi la deve ricostruire.

Si impiega in `recreate-pokemon-distributions-events/STUDIO-03-verifica-del-metodo-sul-corpus.md`.

### 8.11 La struttura a reticolo, e ciò che il test spettrale misura

Le terne di valori consecutivi prodotte da un generatore lineare congruenziale non riempiono uniformemente il cubo: cadono su un numero limitato di piani paralleli. È un risultato generale della famiglia, e non un difetto delle costanti scelte: qualunque scelta di moltiplicatore e incremento produce la medesima struttura, e cambia soltanto il numero dei piani e la loro spaziatura.

La nozione esiste perché individua il limite oltre il quale un generatore di questa famiglia non va impiegato, e quel limite non riguarda il dominio di questo progetto ma va conosciuto per non trasportare la tecnica dove non regge. Se si estraggono `k` valori consecutivi e si interpretano come le coordinate di un punto in `k` dimensioni, i punti così ottenuti giacciono su un reticolo, e il numero dei piani su cui si dispongono cresce come la radice `k`-esima del modulo. Per `k` piccolo i piani sono molti e la struttura è invisibile; per `k` grande sono pochi e la struttura è dominante.

L'esempio svolto è il conto per il generatore del dominio. Con modulo due alla trentaduesima, le coppie di valori consecutivi si dispongono su non più di circa sessantacinquemila piani, che è la radice quadrata del modulo; le terne su non più di circa milleseicento, che è la radice cubica; le decuple su non più di dieci. Ne segue che una simulazione che consumasse dieci valori consecutivi per collocare un punto in dieci dimensioni otterrebbe punti disposti su una decina di iperpiani anziché sparsi, e qualunque conclusione statistica ne uscirebbe distorta.

La ragione per cui questo limite non tocca il lavoro descritto in questo progetto va enunciata, perché è la parte utile del confronto. Qui il generatore non serve a simulare ma a determinare: non si chiede che le sue uscite siano indistinguibili dal caso, si chiede che siano riproducibili, e la struttura a reticolo non ostacola la riproduzione. Il medesimo generatore è inadatto a una simulazione in molte dimensioni e perfettamente adatto a produrre un esemplare, e la differenza è nello scopo e non nel generatore.

Si impiega come limite dichiarato in `docs/index.md`.

### 8.12 Pseudocasuale non significa bianco

Una successione pseudocasuale supera per costruzione alcune prove statistiche e non altre, e la formulazione precisa di che cosa garantisca è la seguente: la distribuzione dei valori è prossima all'uniforme, mentre la successione non è priva di correlazione fra i propri termini. Le due proprietà sono indipendenti, e chiamare casuale una successione perché soddisfa la prima è l'errore più comune su questo argomento.

La nozione esiste perché la differenza fra le due proprietà decide quali usi siano legittimi. Una successione a distribuzione uniforme e correlata va bene dove serve soltanto che i valori coprano l'intervallo con la giusta frequenza, per esempio per scegliere quale esemplare comparirà in un incontro; non va bene dove serve che due valori vicini nella successione siano indipendenti, per esempio in una simulazione o in un uso crittografico. Un generatore lineare congruenziale è nel primo caso e mai nel secondo, e la ragione ultima è la sua invertibilità: chi osserva un'uscita risale allo stato e prevede tutte le successive, che è la definizione stessa di non essere adatto alla crittografia.

L'esempio svolto è la conseguenza per questo progetto, ed è ironica nella direzione giusta. La proprietà che rende quel generatore inadatto alla crittografia, cioè la ricostruibilità dello stato da poche uscite, è esattamente la proprietà su cui questo lavoro si fonda: la ricerca inversa del seme di un esemplare autentico è un attacco elementare a un generatore che non pretende di resistervi. Ne segue che qui non si sta aggirando alcuna protezione, perché non ce n'è alcuna: si sta usando un generatore per ciò che è, e la sua debolezza rispetto a un uso che non ha mai avuto è la nostra strada.

Si impiega in `poke-ace/STUDIO-01-ace-e-legalita-in-home.md` per la distinzione fra un valore imprevedibile e un valore riproducibile.
### 8.13 Identificabilità, e la differenza fra un dato assente e un dato perduto

Un parametro si dice identificabile a partire da un'osservazione quando l'osservazione lo determina in modo unico, cioè quando due valori distinti del parametro non possono produrre la medesima osservazione. È una nozione della teoria della stima e non della crittografia, e la distinzione conta: qui non si chiede se un parametro sia difficile da ricavare, si chiede se sia ricavabile.

La nozione esiste perché separa tre situazioni che la parola irrecuperabile confonde, e che in questo progetto si presentano tutte e tre. La prima è il parametro identificabile e facile, dove l'osservazione lo determina e il calcolo è breve. La seconda è il parametro identificabile e costoso, dove l'osservazione lo determina ma il calcolo richiede una ricerca: la determinazione esiste, e il costo è il solo ostacolo. La terza è il parametro non identificabile, dove nessun calcolo di alcuna durata lo determina, perché l'osservazione è compatibile con più di un valore. Le prime due sono questioni di risorse, la terza è una questione di informazione, e nessuna quantità di ingegno converte la terza nelle prime.

L'esempio svolto è il confronto fra le generazioni sul medesimo campo, cioè i valori individuali di un esemplare da distribuzione, e il confronto dà esiti opposti.

In terza generazione i valori individuali non sono un parametro libero: derivano dal seme attraverso il generatore, insieme al valore di personalità. L'osservazione, cioè l'esemplare, porta trentadue bit di valore di personalità e trenta di valori individuali, mentre il parametro da identificare, cioè il seme, ne ha trentadue o sedici secondo il metodo. I bit osservati eccedono quelli del parametro, quindi il seme è sovradeterminato: è identificabile, e la voce 8.6 mostra che sul seme ristretto è anche immediato. Ne segue che la fedeltà di una ricreazione a un originale è decidibile, perché si riduce al confronto fra due semi, e che è decidibile in un senso forte, poiché non si confrontano i campi visibili ma la causa che li ha prodotti.

Nelle prime due generazioni i valori individuali non derivano da alcun parametro conservato: furono estratti al momento della consegna da uno stato del gioco che nessun campo dell'esemplare registra, e l'esemplare li porta come dato primario. L'osservazione di un esemplare autentico non contiene dunque alcuna informazione su quale fosse quello stato, e nemmeno serve, perché non c'è un parametro comune da ritrovare: due esemplari della medesima distribuzione hanno valori individuali diversi e nessuno dei due è più autentico dell'altro. Il parametro non è nascosto ed è assente, e la fedeltà su quel campo non è decidibile nemmeno in principio.

La formulazione precisa della differenza è dunque questa: in terza generazione la fedeltà è una proprietà verificabile perché la funzione dal parametro all'osservazione è iniettiva nella direzione utile, mentre nelle prime due non lo è, e non lo è perché non c'è alcuna funzione, essendo il campo un ingresso e non un'uscita. Chiamare irrecuperabile il primo caso e il secondo con la medesima parola è l'errore che questa voce serve a evitare.

Ne segue anche l'asimmetria fra le due nozioni che il progetto distingue altrove, cioè legittimità e fedeltà, e vale enunciarla come coppia perché le due generazioni si comportano in modo opposto. Dove il campo deriva da un parametro, la legittimità è difficile e la fedeltà è dimostrabile: difficile perché il verificatore controlla la coerenza fra campo e parametro e ogni errore di ricostruzione la rompe, dimostrabile perché il parametro è identificabile. Dove il campo non deriva da nulla, la legittimità è banale e la fedeltà è impossibile: banale perché qualunque valore del campo è coerente con qualunque altro, non essendoci vincolo da rispettare, e impossibile per assenza del parametro. Le due difficoltà sono complementari e non si presentano mai insieme.

Si impiega in `pokedex-home-completo/SCHEDE-EVENTI-GB.md` e in `recreate-pokemon-distributions-events/STUDIO-03-verifica-del-metodo-sul-corpus.md`.
