...
| se gli amministratori dei quattro server di community accettino un bot di archiviazione in sola lettura | chiedendolo, dichiarando a che cosa serve e quali permessi si chiedono; è gratuito, e un no è un esito legittimo dopo il quale resta la copia manuale |
|---|---|
...
| Token utente di Discord | deciso il 2026-08-26 di NON usarlo, e la decisione resta: automatizzare un account personale è vietato dalle condizioni d'uso e la sanzione è la terminazione dell'account. Il 2026-08-31 è stata però trovata una terza via che il progetto non conosceva, cioè un bot account ufficiale, adottata con ADR-018 | nulla su questa riga: la via del token personale resta chiusa. Ciò che si è aperto sta nella riga seguente |
| Bot account Discord ufficiale | da creare, deciso il 2026-08-31 con ADR-018. Va creata l'applicazione nel portale per sviluppatori, abilitato Message Content Intent, generato l'invito con i soli permessi View Channels e Read Message History, e scritto a mano `DISCORD_BOT_TOKEN` in `.env`, che l'agente non può creare. Lo strumento che lo consuma è `tools/fetch-discord.py`, già scritto e provato nella sua logica con `--self-test` | la lettura automatica dei canali dei server dove il bot venga invitato. Attenzione al limite: dei quattro server che il progetto consulta nessuno è dell'utente, quindi per tutti serve l'autorizzazione di chi amministra, e il primo passo è chiedere |
...
| esiste un bot account Discord invitato in un server | `tools/fetch-discord.py` | legge la cronologia di un canale con impaginazione, cursore per il solo delta e filtri; il token si legge da `.env`, e il presidio rifiuta un token che non sia di un bot |
...
| esportazione verso il template di `fetch-discord.py` e della sezione Discord della regola sulle fonti non recuperabili | chiesta dall'utente il 2026-08-31 e non ancora fatta, per scelta: prima si fissa lo strumento in questo progetto e lo si prova contro il servizio, poi si esporta ciò che ha funzionato | il template ha già la regola, quindi va aggiornata la sezione e aggiunto lo strumento; `catalogo-eventi-gen3.py` invece non si esporta, perché è specifico di questo dominio |
