...
## ADR-057: un rifiuto che dipende dall'istante non decide un nodo, e la diagnosi deve nominare la causa vera

Data: 2026-09-12. Stato: accettata, decisione dell'agente su un difetto misurato.

Il problema. Il lettore del corpus classificava allo stesso modo due rifiuti che non si equivalgono: il divieto scritto in `robots.txt`, che è una proprietà stabile del sito, e l'impossibilità di leggere quel file, che è una proprietà dell'istante in cui la corsa vi è passata. Entrambi producevano un nodo catalogato, cioè deciso in via definitiva, che nessuna ripresa ritenta. Sulla corsa del corpus della collezione del 2026-09-08 questo ha escluso centotrenta nodi su ventotto host, fra cui ottanta pagine di Bulbapedia, dieci di Project Pokemon e tre di Wikipedia, e il censimento generato ne riportava il motivo in una forma che sembrava definitiva. Il lotto di lettura dichiarato ne risultava leggibile per sette voci su ventiquattro.

La causa vera, misurata e non supposta. La richiesta non falliva per un codice di risposta ma nella verifica del certificato. L'archivio delle radici di sistema di questa macchina contiene nove certificati scaduti, e fra essi una copia di ISRG Root X2 scaduta il 15 settembre 2025: i siti che si appoggiano a quell'autorità servono una catena che termina in X2 controfirmata da ISRG Root X1, valida fino al 2035, e la verifica si ferma sulla radice di cui si fida invece di proseguire sul ramo controfirmato. Un browser e `curl` non se ne accorgono perché usano un archivio diverso. Il messaggio che il programma scriveva nominava quindi il sito remoto mentre il difetto stava nella macchina locale, ed è la parte che è costata di più, perché una diagnosi sbagliata chiude la ricerca invece di aprirla.

La decisione, in tre parti. Primo: un rifiuto che dipende dall'istante non decide un nodo ma lo rinvia, cioè lo mette fra i pendenti con il proprio motivo, che è lo stesso trattamento già riservato a ciò che un tetto ha escluso e per la medesima ragione, cioè che l'esclusione non è una proprietà del contenuto. Secondo: la verifica dei certificati usa un archivio di radici corrente quando ne trova uno, e quando fallisce lo dichiara come problema di certificato locale invece di chiamarlo guasto di rete. Terzo: il recupero di una corsa fatta prima della distinzione è un'operazione dello strumento, con `--riprova-transitori` e `--solo-transitori`, e non un intervento a mano sul suo stato, perché il caso tornerà e perché riparare un difetto e proseguire una frontiera sono cose diverse che vanno potute fare separatamente.

Che cosa la decisione rifiuta esplicitamente. Rifiuta di disattivare la verifica dei certificati, che era la scorciatoia disponibile e che avrebbe fatto sparire il sintomo: il difetto stava nell'elenco di chi ci si fida e non nel fatto di fidarsi, e lo strumento non espone alcuna opzione per non verificare, per la stessa ragione per cui non ne espone una per ignorare `robots.txt`. Rifiuta inoltre di trattare un archivio di radici che contenga certificati scaduti come un fatto innocuo in generale: lo è finché nessuna catena viva vi termina, e smette di esserlo appena una vi termina, quindi il collaudo dello strumento verifica quel caso per nome.

La conseguenza di metodo, che vale oltre il caso. La regola sulle fonti non recuperabili prescrive di chiedersi che cosa più tentativi falliti abbiano in comune, e di cercare la via nuova cambiando il capo invece del tragitto. Qui il capo comune non era il sito, che è ciò che il messaggio suggeriva, ma la macchina che chiedeva: il criterio si applica quindi anche un gradino più in basso, cioè agli strumenti e non soltanto alle fonti.

## ADR-058: i marchi sono un asse proprio, distinto dai fiocchi, e nessuno di essi è sotto scadenza

Data: 2026-09-12. Stato: accettata, decisione dell'agente su tre fonti convergenti.

Il problema. Il progetto aveva l'asse dei fiocchi e non quello dei marchi, e non per mancanza di fonti ma perché la rappresentazione li confonde: il formato conservato dal deposito li tiene nella stessa regione di byte, contigui e senza separatore, cosicché una lettura condotta sui byte non incontra alcun confine. Tre fonti lette il 2026-09-12 hanno reso la distinzione impossibile da ignorare, e nessuna delle tre da sola l'avrebbe imposta: i sei incontri titanici, le novantuno edizioni degli incontri a cristallo nero, e i titoli di Pokemon Champions, dove gli stessi marchi compaiono sotto un altro nome perché quel gioco non li disegna.

La decisione. I marchi sono un asse proprio della collezione e non una sottofamiglia dei fiocchi, e il criterio che li separa è il conferimento: un fiocco si conferisce per un merito, cioè per qualcosa che il giocatore ha fatto e che può rifare, mentre un marchio si conferisce per una circostanza dell'incontro, cioè per qualcosa che era vero nel momento in cui l'esemplare è stato trovato. L'enumerazione è `pokedex-home-completo/MARCHI.md`, generata da `tools/marchi.py` leggendo il formato del deposito, e conta cinquantatre marchi su otto byte in sei famiglie, di cui otto appartengono all'insieme che il verificatore della nona generazione tratta a parte.

Il risultato che governa la pianificazione, e che è negativo. Nessun marchio è sotto la scadenza del 26 febbraio 2027, e la ragione è strutturale invece che fortunata: i marchi nascono con l'ottava generazione e vivono nell'ottava e nella nona, che sono titoli per console corrente e parlano al deposito per via diretta. L'asse allarga quindi l'ampiezza della collezione e non la sua urgenza, e nell'ordine per scadenza di ADR-052 va dopo tutto ciò che scade.
...
La decisione, in due parti. La prima è che la fonte di verità della ripresa è `_notes/resume-prompt.md` e nessun'altra: il file alla radice resta cancellato e non si ricrea, e le sue due parti non duplicate, cioè le quattro discipline e i vincoli che non si negoziano, sono state fuse dentro di esso prima della rimozione. La seconda è generale e vale oltre questo caso: non si spezzettano le cose in nuovi file `.md`. Prima di creare un documento nuovo si verifica se il contenuto appartenga a uno che esiste già, e la risposta preferita è che vi appartenga; un documento nuovo si giustifica soltanto quando è generato da uno strumento e ha un `--check` che lo tiene allineato.

Che cosa la decisione costa, e va detto. Il file locale non è versionato, quindi le discipline che vi sono state fuse non arrivano a un clone: è un prezzo accettato dall'utente in cambio della chiarezza, e la mitigazione è che quelle discipline vivono comunque, in forma normativa, nelle regole sotto `.claude/rules/` e nel `CLAUDE.md`, che sono tracciati. La voce di `MAPPA-DOCUMENTI.md` sul file rimosso resta al proprio posto e dichiara la rimozione con il suo perché, perché un documento cancellato in silenzio è indistinguibile da uno dimenticato e la sessione successiva lo ricreerebbe.

## ADR-060: la nona generazione non è posseduta, quindi i suoi marchi si producono
...
La decisione. I marchi di nona generazione non sono voci da cogliere ma voci da produrre, e rientrano in ADR-050 come qualunque altra via non percorribile: il marchio del più forte con le sue specie, e il marchio del titano con i suoi sei incontri. Entrano nella coda quando la coda arriverà alla nona generazione, e non prima, perché ADR-052 ordina per scadenza e questi non scadono. La sezione di `pending.md` che li elencava come finestre è stata riscritta di conseguenza.

L'errore di metodo, registrato perché è il più facile da ripetere. Il possesso di un titolo o di un pezzo di hardware non si assume: si chiede. Assumerlo ha prodotto una consegna operativa inutile, cioè istruzioni per una cattura impossibile, ed è un costo piccolo qui soltanto perché la scadenza era di tre giorni e non di tre mesi.
...
## ADR-062: l'importazione di un salvataggio esterno sul Rubino di prova segue ADR-029, non ne apre una nuova

Data: 2026-09-15. Stato: accettata, decisione dell'utente con estensione dell'agente a un caso non ancora coperto testualmente.

Il problema. L'utente ha proposto di scaricare un salvataggio italiano di Rubino più avanti nella partita e scriverlo sulla cartuccia di prova per avere accesso al PC senza giocare fino a quel punto, autorizzando esplicitamente l'operazione. `hardware-and-perimeter.md` richiede una decisione registrata come ADR prima di importare un salvataggio di terzi, e la domanda è se questo caso ne richieda una nuova o rientri in una già presa.

La decisione. Rientra in ADR-029, che il 2026-09-02 ha già distinto l'importazione di uno stato di avanzamento dall'importazione di un esemplare, per un caso allora di quarta generazione (Parco Amici) e qui esteso alla terza generazione (accesso al PC) per lo stesso principio: nessuno dei due sblocca fa entrare un Pokemon di provenienza altrui nella catena. Le tre condizioni di ADR-029 si applicano identiche: il deposito e la squadra del salvataggio scaricato vanno svuotati prima di scriverlo sulla cartuccia, cosicché nessun esemplare di terzi vi resti; il file va verificato integro e in lingua italiana prima della scrittura; e restano validi il backup in doppia copia e il read-back verificato, perché qui si scrive su una cartuccia vera e non si importa soltanto un file.

La conseguenza, e il motivo per cui questa decisione conta poco nella pratica. Il criterio di triage di ADR-037, cioè che un salvataggio costruito artificialmente non è una fonte di esemplari ma resta un veicolo utilizzabile per uno stato di avanzamento, si applica qui a favore: poiché lo svuotamento del deposito e della squadra è comunque obbligatorio, la qualità o la legittimità degli esemplari che quel salvataggio scaricato porta con sé non contano affatto, perché nessuno di essi arriverà a destinazione. Resta però un dubbio tecnico non affrontato da questa decisione, registrato perché la scrittura effettiva non è ancora avvenuta: se l'iniezione nelle scatole con `tools/carica-lotto-gen3.py`, che opera sui byte del file e non passa dal gioco, renda comunque superflua l'intera operazione, perché la struttura del deposito esiste nel salvataggio indipendentemente da quanta storia sia stata completata.

## ADR-063: Mappa Stinta e Biglietto Aurora si producono sulla cartuccia vera di Smeraldo, dichiarati come prodotti e non come storia

Data: 2026-09-17. Stato: accettata, decisione esplicita dell'utente.

Il problema. Fra le voci della tasca Oggetti Chiave che l'utente ha dettato a voce mancavano la Mappa Stinta (che l'utente chiama Mappa Stinta, per l'Isola Suprema e Mew) e il Biglietto Aurora (per l'Isola Materna e Deoxys), entrambi oggetti reali e compatibili con Smeraldo, distribuiti storicamente da Nintendo via Mystery Gift/e-Reader. Interrogato se li avesse ricevuti per davvero su questa cartuccia, l'utente ha risposto di no, e ha chiesto esplicitamente di inserirli comunque, come se l'evento fosse avvenuto.

La decisione. Si inseriscono, ma il track resta quello che corregge la corruzione dello zaino e questi due oggetti non sono corruzione da correggere: sono contenuto prodotto su richiesta, esattamente nella stessa categoria di ciò che il track `recreate-pokemon-distributions-events` fa altrove nel progetto per gli esemplari da evento, con la stessa distinzione che quel track applica da ADR-049 in avanti fra ottenere e produrre. La differenza che rende la decisione più semplice di quelle sugli esemplari: questi sono oggetti chiave locali al gioco, mai visti da Pokemon Home o Bank, quindi nessuna delle preoccupazioni sul tracciatore e sulle sanzioni online che pesano sul track degli esemplari si applica qui. Il vincolo che li accompagna è di documentazione, non tecnico: `STUDIO-01` e la scheda del sottoprogetto devono dichiarare questi due oggetti come prodotti su richiesta dell'utente il 2026-09-17, mai come restituiti o recuperati da una corruzione, così che una lettura futura di questo studio non li confonda con gli oggetti a cui questa correzione ha ridato la provenienza legittima che avevano prima del bug.

La conseguenza. I due oggetti entrano nella tasca Oggetti Chiave nel prossimo giro di correzione, verificati sul sorgente per il loro identificativo esatto prima della scrittura, con lo stesso rigore (backup doppio, diagnosi, read-back) di ogni altra scrittura su questa cartuccia. Nessuna conseguenza sulla collezione Pokemon Home del progetto, perché gli oggetti chiave non attraversano mai quella catena.

## ADR-065: i tre flag di abilitazione delle isole si accendono scrivendo sul salvataggio

Data: 2026-09-18. Stato: accettata, decisione esplicita dell'utente nella stessa conversazione in cui le alternative sono state esposte.

Il problema. Il Biglietto Eone, il Biglietto Aurora e la Mappa Stinta sono nella tasca Oggetti Chiave della cartuccia vera, ma al porto di Alghepoli nessuna isola compare. La causa è accertata sul sorgente e misurata sul salvataggio, ed è scritta per intero in `gba-save-extraction-smeraldo/STUDIO-03-doni-segreti-e-flag-delle-isole.md`: `CreateLilycoveSSTidalMultichoice` in `src/script_menu.c` di pret/pokeemerald valuta una congiunzione fra l'oggetto nello zaino e un flag di abilitazione, e quel flag lo accende soltanto lo script di consegna del Dono Segreto, che esegue `giveitem` e `setflag` nello stesso punto. Un oggetto scritto nella tasca da fuori ottiene la prima metà della consegna e non la seconda. Per il Biglietto Eone la coppia era già rotta nel dump originale mai toccato, quindi da prima di questo progetto; per gli altri due l'ha prodotta il secondo giro di correzione, autorizzato da ADR-063.

Le alternative considerate. La prima è quella scelta. La seconda era la via ufficiale, cioè iniettare la Carta Meravigliosa originale con NDSEventTool e farsela consegnare dal gioco, che avrebbe avuto il pregio di far accendere il flag al gioco stesso: è stata scartata perché su questa cartuccia richiede comunque due scritture sul salvataggio per aggirare i controlli d'ingresso dello script di consegna (`STUDIO-03` sezione 7), perché richiede un Nintendo DS con scheda di flash, e perché per Smeraldo italiano quello strumento nella versione letta copre soltanto due dei quattro biglietti. La terza era non toccare la cartuccia e procurarsi altrove gli esemplari, scartata perché l'utente vuole gli incontri sulla propria partita.

La decisione. Si accendono `FLAG_ENABLE_SHIP_SOUTHERN_ISLAND` (0x8B3), `FLAG_ENABLE_SHIP_BIRTH_ISLAND` (0x8D5) e `FLAG_ENABLE_SHIP_FARAWAY_ISLAND` (0x8D6) scrivendo sul salvataggio con uno script deterministico, `tools/emerald_event_flags_fix.py`, e si scrive il risultato sulla cartuccia con la procedura di sempre, cioè backup in doppia copia verificato, scrittura con FlashGBX e read-back confrontato per hash.

Il perimetro, che è la parte che qualifica la decisione. Non si accendono i flag `FLAG_RECEIVED_*`, che dichiarano che un dono è stato consegnato dal Dono Segreto: quella consegna non è avvenuta, accenderli scriverebbe un fatto falso, e il menu del porto non li guarda. Non si accendono i `FLAG_SHOWN_*`, che il gioco accende da sé. Non si accende `FLAG_ENABLE_SHIP_NAVEL_ROCK`, perché il Biglietto Magico non è nella tasca e un flag senza il suo oggetto ricostruirebbe la coppia rotta al contrario; lo strumento porta una precondizione che lo impedisce anche per errore. E si registra che cosa la decisione produce davvero: non fabbrica alcun esemplare e non scrive nulla nel deposito, ma rende raggiungibili tre luoghi dove gli incontri avverranno poi normalmente. Gli esemplari saranno quindi ottenuti giocando, e l'unica cosa prodotta da fuori resta il permesso di salire sulla nave.

La conseguenza sulla provenienza, coerente con ADR-063 e con la distinzione fra legale e legittimo che il track `poke-ace` ha fissato. Il permesso di accesso alle tre isole è contenuto prodotto e va dichiarato come tale ovunque se ne parli, mai presentato come storia della partita.

## ADR-064: convenzione di nome fissa per i file di salvataggio di Smeraldo

Data: 2026-09-17. Stato: accettata, direttiva esplicita dell'utente.

Il problema. Nel secondo giro di correzione i file intermedi sono stati nominati con comodo (`round2.sav`, `round2-b.sav`, `round2-CORRETTO.sav`), abbandonando senza dichiararlo la convenzione già in uso nel primo giro, che codifica allenatore, ID allenatore e tempo di gioco. L'utente ha richiamato la convenzione originale e ha chiesto di non discostarsene mai più.

La decisione. Ogni file di salvataggio di questo sottoprogetto si chiama sempre `Pokemon - Versione <Gioco> (Italy) - <ALLENATORE>-<ID allenatore>-<tempo di gioco>hXXm-<data>`, con `-CORRETTO` e `-READBACK` come soli suffissi ammessi per le varianti dello stesso dump. Il tempo di gioco nel nome è quello letto davvero dal file (`SaveBlock2`, offset 0x0E ore e 0x10 minuti, già noto da `verifica-salvataggi.py`), mai copiato da un giro precedente: due dump dello stesso giorno possono avere tempi di gioco diversi, come è successo qui (788h36m al primo giro, 788h44m al secondo, perché l'utente ha continuato a giocare fra l'uno e l'altro). Una copia di verifica ridondante (una seconda lettura fatta solo per confrontarne l'hash con la prima) non prende un nome proprio e non resta su disco oltre il controllo: o è identica, e allora è superflua, o non lo è, e allora il backup non è verificato e si rilegge da capo.

La conseguenza. I file del secondo giro sono stati rinominati secondo questa convenzione su entrambi i dischi, e ogni riferimento nei documenti tracciati (`STUDIO-01`, `pending.md`, la scheda del sottoprogetto) è stato aggiornato di conseguenza nello stesso giro in cui il rinomino è avvenuto, per `chat-non-e-memoria.md`. Le due copie di verifica ridondanti del secondo giro sono state cancellate.

## ADR-066: gli eventi delle quattro isole si riaprono per intero sulla cartuccia vera di Smeraldo

Data: 2026-09-18, a sera. Stato: accettata, decisione esplicita e riaffermata dell'utente dopo che l'obiezione era stata esposta una volta.

Il problema. Il terzo giro (ADR-065) ha acceso i flag di abilitazione e il porto di Alghepoli offre adesso le tre isole, ma sbarcando due su tre sono vuote: sull'Isola Materna manca il triangolo del rompicapo e sull'Isola Suprema manca Mew, mentre sull'Isola Remota Latios c'è. La causa è una seconda famiglia di flag, indipendente da quella dei biglietti, che ogni mappa di destinazione consulta nel proprio script di transizione per decidere se scoprire l'esemplare, e che su questa partita dichiara già avvenuti gli incontri di Deoxys, Mew, Lugia e Ho-Oh. Il Monte Cordone, in più, non compare affatto perché il Biglietto Magico non è nella tasca. Tutto in `gba-save-extraction-smeraldo/STUDIO-03-doni-segreti-e-flag-delle-isole.md` dalla sezione 14.

Il fatto che qualifica la decisione, e che non era disponibile quando ADR-065 fu presa. Che quei flag non siano stati accesi giocando questa partita non è più un'ipotesi ma una dimostrazione, e sta nella sezione 16 dello stesso studio. Nessuno script di pret/pokeemerald esegue mai un `clearflag` su un `FLAG_ENABLE_SHIP_*`, quindi quei flag sono monotoni; misurati spenti nel dump originale mai toccato non erano mai stati accesi; senza di essi il menu del porto non offre quelle isole e le loro mappe sono irraggiungibili; quindi i flag che vi si accendono non possono essersi accesi per gioco. E in aggiunta `FLAG_CAUGHT_MEW` acceso insieme a `FLAG_ARRIVED_ON_FARAWAY_ISLAND` spento è una coppia che il gioco non può produrre, perché l'unico ingresso alla mappa dove Mew si cattura passa da una mappa la cui transizione accende quel secondo flag senza condizioni.

La decisione. Si riaprono tutti e quattro gli eventi, non due, e si completa anche il Monte Cordone. L'utente lo ha chiesto in modo esplicito e ha respinto l'obiezione implicita nella proposta di limitarsi a Deoxys e Mew, dichiarando che il possedere già Lugia e Ho-Oh altrove non è la questione e che ciò che vuole è poter rigiocare l'evento completo su questa cartuccia. L'obiezione era stata esposta una volta con i suoi termini reali, la riaffermazione è registrata qui, e non va ripetuta ai passi successivi.

Le tre operazioni, nel loro ordine obbligato. Prima si aggiunge il Biglietto Magico (id 370) alla tasca Oggetti Chiave con `tools/emerald_key_item_add.py`, che scrive la quantità mascherata con la chiave di sicurezza come il gioco si aspetta. Poi si accende `FLAG_ENABLE_SHIP_NAVEL_ROCK` (0x8E0) con `tools/emerald_event_flags_fix.py`, il cui piano è stato esteso alla quarta riga proprio in virtù di questa decisione: al terzo giro quella riga non c'era perché la precondizione dello strumento, che pretende l'oggetto in tasca, l'avrebbe fermata, ed è la ragione per cui l'ordine conta. Infine si spengono i flag di testa dei quattro incontri con `tools/emerald_encounter_flags_fix.py --tutti`, cioè `FLAG_BATTLED_DEOXYS` (0x1AD), `FLAG_CAUGHT_MEW` (0x1CA), `FLAG_CAUGHT_LUGIA` (0x091) e `FLAG_CAUGHT_HO_OH` (0x092), insieme ai quattro `FLAG_DEFEATED_*` corrispondenti che sono già spenti e che lo strumento riferisce come tali.

Il perimetro, che è la parte che qualifica la decisione come le due che la precedono. Non si toccano i `FLAG_HIDE_*`, perché li gestisce da sé lo script di transizione della mappa quando trova il flag di testa spento: toccarli sarebbe scrivere più del necessario. Non si tocca il Pokedex, che è un'altra struttura del salvataggio e che questi flag di evento non governano: nessuna specie viene tolta e la percentuale di completamento non cambia. Non si tocca nulla di zaino, squadra, denaro e deposito oltre alla singola voce nuova nella tasca Oggetti Chiave, e la verifica lo conferma con una diagnosi identica riga per riga tranne quella voce. E non si accendono i `FLAG_RECEIVED_*` né i `FLAG_SHOWN_*`, per le stesse ragioni di ADR-065.

La conseguenza sulla provenienza. Il Biglietto Magico è contenuto prodotto e va dichiarato come tale ovunque se ne parli, esattamente come il Biglietto Aurora e la Mappa Stinta per ADR-063, e il permesso di accesso alle isole resta contenuto prodotto per ADR-065. Gli esemplari no: quelli li genera il gioco durante l'incontro, e vale registrare il meccanismo perché è ciò che rende la distinzione verificabile e non dichiarativa. Il comando di script `seteventmon`, che tutte e quattro le mappe eseguono prima della lotta, porta a `CreateEnemyEventMon` e quindi a `CreateEventMon`, che in `src/pokemon.c` di pret/pokeemerald imposta `MON_DATA_MODERN_FATEFUL_ENCOUNTER` a vero. Deoxys, Mew, Lugia e Ho-Oh catturati su quelle isole porteranno quindi il contrassegno di incontro fatidico che un verificatore si aspetta di trovare su un esemplare di quella provenienza: non somiglieranno a esemplari legittimi, lo saranno, perché a costruirli è il gioco.

La conseguenza sull'altro track. `pokedex-home-completo` guadagna una porta che non passa da un lotto composto, ed è la prima di questo genere: Deoxys e Mew, che nel deposito di questa cartuccia non ci sono e che l'ADR-032 conta fra le specie prive di incontro nei titoli a via diretta, diventano ottenibili giocando sulla propria cartuccia. Lugia e Ho-Oh ci sono già dalla distribuzione italiana "10ANNI" su Rubino, quindi per loro la porta è un di più.

La conseguenza sull'esecuzione. Questa scrittura non si fa da sola: per direttiva dell'utente si unisce al giro che mette insieme la Grotta Mutevole con il Succodibacca e la pulizia del deposito PC, perché ogni scrittura sulla cartuccia è un rischio che non conviene ripetere. Restano da scegliere, e sono le sole cose che mancano al giro, il valore di `VAR_ALTERING_CAVE_WILD_SET` e gli oggetti rari con cui sostituire i sette comuni del deposito. Valgono come sempre il backup in doppia copia verificato, il dump di controllo prima di scrivere e il read-back confrontato per hash.

## ADR-067: la squadra del Parco Lotta si genera in una pipeline sintetica e in più si scrive nei box della cartuccia vera

Data: 2026-09-21. Stato: accettata.

Il problema. Il 2026-09-21, aprendo il track Battle Frontier, si era posta la stessa domanda già decisa da `poke-ace` per la collezione verso Home: fabbricare esemplari per via di codice invece che giocarli è legittimo solo dove nessuna via legittima esiste. Per la squadra del Parco Lotta la risposta era diversa da quella dell'Altering Cave, perché quegli esemplari non attraverseranno mai Home: il rischio che `poke-ace` descrive (Home traccia la provenienza a parte, sanzione la sospensione dell'account) non si applica a un box che resta locale alla cartuccia. Restava però aperta una seconda domanda, distinta dalla prima: dove vivono gli esemplari fabbricati, cioè se in una pipeline di sintesi separata dal salvataggio fisico o direttamente nei box della cartuccia vera di Alex. Le due risposte dell'utente su questo punto erano state in apparente contraddizione: la domanda strutturata aveva scelto la pipeline separata, un passaggio successivo parlava di "box dedicati" e di riorganizzare quelli esistenti.

La decisione, chiesta di nuovo esplicitamente. Entrambe le cose, non l'una o l'altra. La pipeline sintetica, che riusa la sintesi software già in uso per il ponte gen3-gen4 e per il Parco Amici (`pokebridge/gen3.py`, `parco_amici.py`), genera e valida gli esemplari: moveset in stile Smogon con i PP di ogni mossa massimizzati come se fosse stato usato PP Max su ciascuna, statistiche e natura coerenti con un allenamento reale, marcatura di provenienza sintetica dichiarata e non nascosta. Una selezione da quella pipeline si scrive poi anche nei box della cartuccia vera, perché è lì che si gioca davvero il Parco Lotta: è un passo distinto, successivo, con la stessa disciplina di backup doppio, dump di controllo e read-back di ogni altra scrittura su questa cartuccia.

La conseguenza. Il generatore da costruire non è uno strumento solo ma due fasi collegate: la sintesi, che produce e valida, e l'iniezione nei box, che è un tipo di scrittura mai fatto finora su questo salvataggio (le correzioni precedenti hanno sempre toccato zaino, flag e checksum, mai la struttura di un esemplare nei box). La seconda fase richiede quindi una verifica byte per byte nuova, non il riuso diretto degli strumenti già scritti. Resta fuori, e lo è per costruzione, qualunque esemplare fabbricato destinato a Home: quella via resta quella di `poke-ace`, non toccata da questa decisione.
## ADR-068: una squadra per edificio, e alla Piramide una per giro

Data: 2026-09-21. Stato: accettata. Supera la domanda aperta lasciata da ADR-067 e dalla sezione 6 di `STUDIO-04`.

Nota di correzione, aggiunta lo stesso giorno dopo il commit `2be0762`. La prima stesura di questa voce nominava gli edifici con nomi tradotti dall'inglese invece che verificati sul gioco italiano, e quattro su sette erano sbagliati: Fabbrica per Azienda Lotta, Torneo per Cupola Lotta, Arena per Dojo Lotta, Piccozza per Serpe Lotta. I nomi sono stati corretti in questa voce benche' il registro sia append-only, perche' un nome proprio sbagliato non e' una decisione da superare con una voce nuova ma un fatto errato che renderebbe la voce inutilizzabile davanti al gioco; la decisione non e' cambiata di una virgola. La fonte dei nomi corretti e' `wiki.pokemoncentral.it`, ed e' registrata in `SOURCES.md` e nella sezione 3 di `STUDIO-04`.

Il problema. `STUDIO-04` aveva misurato che i sette edifici del Parco Lotta non sono sette gradini della stessa scala e che una squadra sola non li copre: al Palazzo Lotta le mosse le sceglie la natura, e lo Slaking Allegro della squadra gia' indicata dall'utente vi sceglie l'attacco il trentacinque per cento delle volte sia sopra sia sotto la meta' dei punti salute. Ne discendeva una domanda che era dell'utente e non dell'agente, perche' decide quanto lavoro di generazione il progetto si assume: una squadra sola per i sette edifici, che i numeri dicono impraticabile, oppure una per gruppo di edifici.

La decisione. L'unita' di progettazione e' la coppia edificio-squadra, e dove un edificio cambia bestiario da una serie all'altra l'unita' scende ancora, alla coppia giro-squadra. In concreto: una squadra dedicata per ciascuno dei sei edifici in cui si porta una squadra, cioe' Torre, Cupola Lotta, Arena, Palazzo, Serpe Lotta e Piramide; nessuna squadra per l'Azienda Lotta, dove si combatte con esemplari in prestito; e alla Piramide Lotta una squadra per giro, perche' il gioco sostituisce l'intero bestiario selvatico ogni sette piani secondo un ciclo di venti giri, verificato sul sorgente in `src/data/battle_frontier/battle_pyramid_level_50_wild_mons.h` e nel calcolo `(serie / 7) % 20` di `src/battle_pyramid.c`. Per il solo simbolo d'oro della Piramide, che arriva a settanta piani, i giri da coprire sono i primi dieci piu' l'undicesimo in cui compare Brandon.

La conseguenza sul lavoro. Il fabbisogno di esemplari non e' piu' una squadra di sei ma un catalogo: cinque squadre di tre per gli altri edifici piu' fino a undici squadre di tre per la Piramide, ciascuna in doppia copia per lo scambio gia' concordato con una terza persona. Il vincolo di iscrizione che vieta due esemplari della stessa specie o due volte lo stesso strumento vale dentro una squadra e non fra squadre diverse, quindi lo stesso esemplare puo' essere riusato in piu' edifici quando serve; alla Piramide, dove nessun esemplare puo' tenere uno strumento, il vincolo sugli strumenti non si applica affatto. Ne discende che la pipeline sintetica di ADR-067 non produce un lotto ma una famiglia di lotti parametrata sull'edificio e, per la Piramide, sul giro, e che la riorganizzazione dei box della cartuccia vera va dimensionata su questo numero e non su sei esemplari.

L'alternativa scartata, e perche'. Una squadra sola, o due squadre per gruppi di edifici, sarebbe stata piu' economica da generare e da tenere nei box, ed e' la forma in cui quasi tutte le squadre pubblicate sui forum esistono, perche' sono pensate per la Torre. E' stata scartata sui numeri e non per gusto: al Palazzo la stessa squadra rifiuta di attaccare, alla Piramide non puo' tenere strumenti e affronta un bestiario che cambia per intero ogni sette piani, e alla Serpe Lotta il criterio di sopravvivenza non e' la potenza ma l'immunita' a cinque alterazioni di stato con probabilita' dichiarate. Tre edifici su sette invalidano quindi il presupposto dell'ipotesi economica.
## ADR-069: la discussione Smogon 2246 si abbandona, e il debito di lettura del Parco Lotta si dichiara chiuso

Data: 2026-09-21. Stato: accettata.

Il problema. La lettura integrale delle fonti del Parco Lotta ha lasciato fuori una sola voce su circa quaranta: la discussione Smogon numero 2246, che risponde quattrocentotre' anche con uno user agent da browser perche' sta in un forum archiviato del sito. Non e' un muro di credenziali, quindi le vie che `web-sources-not-fetchable.md` elenca dopo il recupero locale non si applicano nel modo consueto: l'archivio pubblico di terzi non copre quel dominio, `claude-in-chrome` aprirebbe una scheda su una pagina che rifiuta anche a un browser a meno che l'utente non abbia accesso a quell'archivio, e la consegna manuale richiederebbe che l'utente possa aprirla, che e' precisamente cio' che non si sa.
...
Contesto. Il proprietario vuole che questa cartuccia porti tutti gli esemplari di terza generazione che il progetto ha generato, come era stato fatto sul Rubino di prova, restando legittima. I lotti contano 209 esemplari, cioe' 176 eventi in tutte le lingue, 19 scambi in gioco e 14 incontri da biglietto, piu' il Lotad e il Seedot piu' grandi possibili, per 211 posti; il deposito ne ha 420, di cui 64 al lotto del Parco Lotta e 241 alla collezione dopo ADR-075.

Decisione del proprietario. I 27 cromatici del codice Action Replay si cancellano, perche' non li ha catturati ne' lui ne' Matteo; le catture vere restano. Delle catture vere si tiene per ogni specie la piu' vecchia, cioe' la prima nel deposito, e si tolgono le 64 copie in piu'. Gli eventi restano tutti, in tutte le lingue, perche' averli da varie parti del mondo e' parte del valore della collezione. Gli incontri da biglietto entrano come catturati da allenatori diversi da ALEX, ciascuno con la propria partita, e scambiati: e' la storia di un amico che gioca l'evento sulla propria cartuccia, e un allenatore diverso per ogni esemplare, perche' ogni partita usa il biglietto una volta sola. I 5 posti che ancora mancano si prendono dalla copia 2, quella per lo scambio, delle cinque riserve meno usate nei thread, cioe' Marowak, Regirock, Steelix, Dusclops e Scizor, le cui copie restano come file.

Conseguenze. Il deposito arriva a 420 esemplari su 420, cioe' pieno: per catturare Mew e Deoxys in gioco serve posto in squadra. Esemplari di prima e seconda generazione non entrano, perche' non possono salire alla terza per alcuna via del gioco, e lo stesso vale in discesa per la quarta e la quinta. Diciotto specie presenti soltanto fra i cromatici forzati escono dalla collezione con loro. Il verificatore giudica l'intero deposito prima della scrittura.

## ADR-075: come si risanano i 40 irregolari di Smeraldo, e gli sfondi dei box

Data: 2026-09-23. Stato: accettata, attuazione in corso. Riapre, su richiesta del proprietario di chiudere Smeraldo per intero prima di tornare a HOME, il risanamento che ADR-074 prevedeva e che era stato rimandato a dopo HOME.

Contesto. Nel deposito, dopo il riordino di ADR-074, restano 40 esemplari che PKHeX giudica irregolari: 11 copie livello 100 del noleggio dell'Azienda Lotta nel box 1, 26 cromatici prodotti dal codice Action Replay fra box 2 e box 8, un Seedot e un Wurmple con la Safari Ball su un incontro fuori dalla Zona Safari. I cromatici del codice portano una personalita' che il gioco non puo' produrre accanto ai loro valori individuali, e un identificativo segreto alterato, come registrato in STUDIO-02 sezione 3.

Decisione del proprietario, in quattro parti. I 26 cromatici restano cromatici: ciascuno si rigenera con un seme reale del primo metodo che sia cromatico per l'identificativo e il segreto veri dell'allenatore, conservando specie, livello, luogo e livello d'incontro, sfera, mosse, esperienza, punti base, strumento e natura, mentre personalita' e valori individuali diventano quelli del seme. Le 11 copie del noleggio si cancellano, perche' il gioco non le consegna mai. Seedot e Wurmple ricevono una Poke Ball, che e' il loro unico campo sbagliato. Gli sfondi si scrivono nello stesso giro: nove sfondi ordinari diversi sui box 1-9, Plain sul 10 e sull'11, lo sfondo Amici sul 12, 13 e 14; lo sfondo Amici gia' sbloccato nella partita, motivo Legendary con icona Bolt, si conserva.

Conseguenze. La scrittura e' una sola e porta insieme il risanamento, gli sfondi e l'ottavo giro del lotto, che corregge l'amicizia degli esemplari con Ritorno. Dopo la cancellazione il deposito si ricompatta, cosi' che il box 1 non resti con undici buchi. Ogni esemplare risanato passa dal giudizio di PKHeX sul file lavorato prima di andare sulla cartuccia, sotto la disciplina intera di `hardware-and-perimeter.md`.

## ADR-074: il deposito si riordina per intero, gli esemplari irregolari si risanano, e il lotto va negli ultimi tre box

Data: 2026-09-23. Stato: accettata, attuazione da fare. Supera, per la sola collocazione, la scelta del box 5 e del box 10 registrata in STUDIO-05 sezione 8 a supporto di ADR-067.

Contesto. Il lotto del Parco Lotta e' cresciuto da quindici a trentadue esemplari in doppia copia, cioe' sessantaquattro posizioni, e non entra piu' nei due box previsti. Il deposito della cartuccia ha 252 esemplari sparsi in quattordici box con nomi eterogenei, e nel dump del 2026-09-17 quarantadue esemplari portavano il triangolo rosso: undici copie del noleggio dell'Azienda nei box PARKLOTT, ventisette cromatici del codice Action Replay nel box SHINY, due con la Safari Ball su un incontro ordinario, e Doduo e Paras di AXEL, cancellati in seguito.

Decisione del proprietario, in cinque parti. Le copie restano due per esemplare, e il lotto occupa un terzo box. Gli esemplari gia' presenti si compattano nei primi box, i sessantaquattro del lotto vanno nei box 12, 13 e 14, prima gli otto titolari delle squadre e poi le ventiquattro riserve, con le due copie di ciascun esemplare affiancate. I quattordici box prendono il nome predefinito del gioco, da BOX 1 a BOX 14. Gli esemplari irregolari li risana l'agente, rigenerandoli legali, con le scelte che toccano l'identita' di un esemplare, come la cromaticita' di quelli del codice, mostrate al proprietario caso per caso prima di scriverle. Gli sfondi dei box si personalizzano, compreso quello speciale della categoria Amici.

Conseguenze. La scrittura sulla cartuccia diventa una sola operazione piu' larga invece di due piccole, e resta sotto la disciplina intera di `hardware-and-perimeter.md`: estrazione nuova della cartuccia, backup in doppia copia, lavoro sul salvataggio in un file separato, verifica in PKHeX di tutto il deposito prima della scrittura, rilettura byte per byte dopo. Sugli sfondi il sorgente fissa un limite che va detto prima di progettarli: il salvataggio conserva un solo sfondo Amici, `struct WaldaPhrase` in `SaveBlock1`, quindi piu' box possono usarlo ma mostrano tutti lo stesso disegno, e gli altri sedici sono gli sfondi ordinari del gioco. Lo sfondo Amici si ottiene in gioco a Ferrugipoli, `RustboroCity_Flat1_2F`, con una frase di quindici lettere che dipende dall'identificativo dell'allenatore, e l'algoritmo che la valida sta in `src/walda_phrase.c`: la frase si puo' quindi calcolare qui invece che su un generatore esterno, e inserirla in gioco resta la via che non scrive nulla sulla cartuccia.

## ADR-073: Suicune e Raikou vengono da Colosseum, e il generatore imparera' la correlazione di Colosseum e XD

Data: 2026-09-22. Stato: accettata, implementazione da fare.

Contesto. Suicune e Raikou erano dichiarati come vaganti di Rosso Fuoco, e quella provenienza porta due difetti indipendenti che il verificatore rifiuta. Il primo e' il livello di incontro pari al tetto del Parco Lotta, ed e' rientrato da solo con la correzione sull'esperienza registrata lo stesso giorno. Il secondo no: i valori individuali di un vagante sono conservati su otto soli bit in tutti i giochi tranne Smeraldo, cioe' cinque per la salute e tre per l'attacco e zero per tutto il resto, e il verificatore lo pretende invece di ignorarlo. Un Suicune vagante legittimo ha quindi difesa, attacco speciale, difesa speciale e velocita' a zero, che al Parco Lotta non regge.

Le vie erano tre e sono state esposte al proprietario con i rispettivi costi: accettare il vagante com'e', insegnare al generatore la provenienza di Colosseum, oppure lasciare i due fuori dal lotto del livello cinquanta e tenerli per il livello aperto.

Decisione. Si insegna al generatore la provenienza di Colosseum. L'esemplare Ombra si incontra a livello quaranta, quindi ha dieci livelli di esperienza per l'allenamento, e non ha valori individuali troncati.

Conseguenze. La lettura del sorgente ha ridotto molto il costo previsto, e vale scriverlo perche' la stima iniziale era pessimistica per una ragione che si poteva verificare prima di stimare. Gli esemplari Ombra di Colosseum e XD portano in generale un vincolo sul valore di personalita' legato alla squadra dell'avversario da cui li si sottrae, che si verifica con una ricerca in profondita' all'indietro sui fotogrammi del generatore: e' l'algoritmo che rendeva la via cara. Suicune e Raikou, pero', usano la squadra dichiarata vuota nelle tabelle del verificatore, e il controllo dei vincoli esce subito con esito positivo quando la squadra e' vuota. Il loro unico requisito e' quindi la correlazione propria di Colosseum e XD, cioe' un generatore lineare con moltiplicatore e incremento diversi da quello delle cartucce, che dal seme produce nell'ordine la prima parola dei valori individuali, la seconda, la meta' alta della personalita' e la meta' bassa. E' la stessa struttura di ricerca rovesciata che il progetto ha gia' per il primo metodo, con costanti diverse e un ordine di chiamate diverso.

Il lavoro non serve solo a questi due esemplari, ed e' la seconda ragione della decisione: tutto cio' che arrivera' da Colosseum e da XD nei lotti per il deposito passera' di qui. Il proprietario ha inoltre aperto una verifica connessa da fare quando si tornera' all'obiettivo principale, cioe' se esistano specie ottenibili o trasferibili soltanto da quei due titoli, che e' una domanda di completezza sull'enumerazione e non su questo lotto.

## ADR-072: il sorgente di un oracolo libero si legge prima di chiedere una schermata

Data: 2026-09-22. Stato: accettata.

Contesto. Il lotto del Parco Lotta era fermo da due giri su sei esemplari conformi su sessantaquattro, con due famiglie respinte, cioe' le uova e le statiche di livello cinquanta, e nessuna causa leggibile in alcuna delle ottantatre colonne del dump dei box. La conclusione registrata al terzo giro era che la motivazione andasse chiesta al verificatore nella forma in cui lui la espone, cioe' il referto per singolo esemplare, e quindi al proprietario sotto forma di schermata. Quella conclusione era giusta a meta'. PKHeX e' un programma libero e il suo sorgente e' pubblico: le regole che applica non stanno soltanto nella sua interfaccia, stanno nel suo codice, e leggerle costa due richieste di rete e non costa niente a nessuno.

Decisione. Quando un oracolo esterno e' un programma libero, la sua fonte precede la sua interfaccia. Si legge il codice della regola prima di chiedere a una persona di riprodurre il verdetto su uno schermo. La richiesta di una schermata resta la via corretta quando l'oracolo e' chiuso, quando il codice non e' raggiungibile, oppure quando la lettura del codice non ha prodotto una spiegazione che regga.

Conseguenze. Le due cause sono state trovate in una sessione, senza attendere nulla, e sono risultate di natura diversa fra loro: le uova cadevano per una correlazione fra personalita' e valori individuali che per un esemplare allevato non deve esistere, e le statiche per un vincolo sui punti base che discende dall'assenza di esperienza guadagnata. La seconda ha corretto anche una convinzione del progetto, cioe' che un esemplare consegnato dal gioco al livello di gioco fosse la provenienza piu' pulita: e' invece la piu' vincolata, perche' chi nasce al tetto non puo' allenarsi. La regola generale che ne discende, ed e' la ragione per cui questo ADR esiste, e' che la stessa gerarchia delle fonti che il progetto applica alla conoscenza di gioco, cioe' il sorgente prima dell'enciclopedia, si applica anche agli strumenti che giudicano il lavoro.

## ADR-071: la cartella dei report di milestone si rimuove, e il suo contenuto resta nella tesi

Data: 2026-09-22. Stato: accettata ed eseguita dall'utente.

Contesto. La cartella `reports/` era nata il 2026-08-25 per ospitare un report in LaTeX per ogni traguardo chiuso, con taglio da ingegneria delle telecomunicazioni, e ne ha ospitato uno solo, quello sulla milestone dei formati e del lato Game Boy. Nel frattempo la tesi ha assorbito quella materia e l'ha estesa: il confronto fra l'inserimento di byte in banda e la correzione fuori banda, il calcolo del tempo di trasferimento del blocco e la parentela fra il checksum di sezione della terza generazione e quello di Internet stanno oggi nel capitolo sul cavo link e nell'appendice sui codici, con una trattazione piu' ampia di quella del report. La cartella teneva quattro file tracciati e una dozzina di derivati di compilazione non tracciati.

Decisione. La cartella si rimuove per intero invece di essere reindirizzata o svuotata con un rimando. Il contenuto resta dove e' gia': `tesi/capitoli/10-cavo-link.tex` e `tesi/capitoli/A5-appendice-codici.tex`.

Conseguenze e motivazioni. La sola dipendenza dichiarata altrove era il commento in testa a `tesi/preambolo.tex`, che dice di derivare da `reports/preamble.tex`: quella derivazione e' avvenuta una volta ed e' storica, non viva, quindi il commento si riscrive per dire che il vincolo sulla TinyTeX minimale resta, senza puntare a un file che non c'e' piu'. L'indice dei satelliti nel `CLAUDE.md` perde la propria riga. La forma dei report di milestone non viene abbandonata come idea: se un traguardo futuro meritera' un documento a se', lo si scrivera', ma dentro la tesi come capitolo o appendice invece che in una cartella parallela, perche' due luoghi per la stessa materia sono il modo in cui una delle due copie invecchia senza che nessuno se ne accorga. La storia del repository conserva comunque il report, quindi nulla e' perduto.

## ADR-070: le quattro scelte che chiudono la composizione delle squadre del Parco Lotta

Data: 2026-09-21. Stato: accettata. Emenda ADR-068 sul solo punto della Piramide Lotta e chiude una pendenza aperta dal 2026-09-17.

Il problema. `STUDIO-05` aveva composto le sei squadre e lasciato aperte quattro domande che non erano dell'agente: due decidevano quanto lavoro di generazione il progetto si assume, una riguardava una specie scelta contro il materiale dell'utente, e una era rimasta in sospeso da settembre. Sono state poste in forma esplicita, con le alternative e il loro costo accanto, perche' una prima esposizione non le aveva rese riconoscibili come domande.

La prima decisione, sulla Piramide Lotta. Una squadra sola che gioca tutti i giri cambiando l'ordine di conduzione, e non una squadra per giro come ADR-068 prescriveva alla lettera. L'utente ha aggiunto una condizione che allarga l'ambito invece di restringerlo: dove esistano informazioni anche per i giri oltre il decimo, cioe' quelli che servono a una serie da record e non al solo simbolo d'oro, si coprono anche quelli. I giri sono venti e la guida ne copre dieci, quindi i restanti dieci sono stati calcolati e non dedotti a occhio, con lo strumento `parco_lotta_piramide_ordine.py`.

Il presidio su quel calcolo va registrato perche' e' la ragione per cui il risultato e' utilizzabile e insieme il limite entro cui lo e'. Lo strumento calcola anche i dieci giri di cui la guida parla e confronta: indica un esemplare che la guida impiega davvero in quel giro in sette casi su dieci, e indovina quale dei due conduca in quattro su dieci. Il secondo numero dice che l'ordine fra i due membri impiegati resta materia di campo; il primo basta all'uso che se ne fa. Una prima versione dello strumento confrontava i soli tipi e ricostruiva la guida in due casi su dieci: e' stata sostituita da un calcolo di danno, perche' il criterio che la guida dichiara e' abbattere in un colpo, e un colpo efficace ma debole non abbatte.

La seconda decisione, sulla legittimita' a livello 50, che era aperta dal 2026-09-17. Il criterio e' l'ottenibilita' in terza generazione e non su questa sola cartuccia: sono ammessi gli scambi da Rosso Fuoco, Verde Foglia, Colosseum e XD, quindi anche mosse e provenienze che Smeraldo da solo non offre. Resta escluso tutto cio' che un verificatore giudicherebbe illegale, e resta fuori dall'ambito qualunque esemplare destinato a Pokemon Home, che ADR-067 aveva gia' separato.

La terza decisione, su Blissey alla Serpe Lotta. Confermata, benche' fosse la sola specie del catalogo a non venire dal materiale dell'utente. La ragione e' che compare nelle due serie piu' lunghe dichiarate per quell'edificio, cinquecentosessanta e quattrocentoventi sale, e che li' non si vince infliggendo danno ma sopravvivendo.

La quarta decisione, sui sette esemplari fuori dalle sei squadre. Milotic, Starmie, Gengar, Salamence, Suicune, Snorlax e Heracross si generano subito insieme agli altri otto invece di restare una riserva sulla carta. La ragione e' operativa: quando una squadra si rompe sul campo la correzione e' la sostituzione di un esemplare, e averla gia' nella cartuccia significa riprovare la sera stessa invece di aprire una corsa di generazione. Il loro insieme di mosse resta dichiarato provvisorio, perche' e' stato deciso senza sapere contro che cosa serviranno.

La conseguenza sui numeri, che e' il punto in cui questa voce diventa operativa. Quindici esemplari distinti invece di otto, trenta slot invece di sedici con la doppia copia, cioe' esattamente un box su quattordici. Contro i novantasei che ADR-068 applicata alla lettera avrebbe richiesto. Il catalogo e' `gba-save-extraction-smeraldo/squadre-parco-lotta.json`, la verifica la fa `parco_lotta_valida_squadre.py`, e il solo rilievo aperto e' che Heracross richiede un livello reale di almeno cinquantatre perche' conosca Megahorn, che il Parco ricalcola comunque a cinquanta.
