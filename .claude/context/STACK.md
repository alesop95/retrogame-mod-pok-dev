...
La console è un Old 3DS XL con seriale che inizia per SEH, partito dal firmware 11.17.0-50E, oggi su custom firmware, con una scheda SD da 32 GB formattata FAT32. Il lettore di cartucce Game Boy e Game Boy Advance è un GBxCart RW v1.4 Pro nella variante USB-C, colore blu, ordinato il 18 agosto 2026 con il cavo USB-A verso USB-C e arrivato il 2026-09-09. Collegato il 2026-09-15: il cancello dei driver CH340/CH341 è superato, risolto in automatico da Windows Update (pacchetto del produttore "wch.cn - Ports" 3.9.2024.9, servizio kernel `CH341SER_A64`) senza il download manuale da wch-ic.com che questa scheda dava per previsto. Il dispositivo compare in Gestione Dispositivi come "USB-SERIAL CH340 (COM5)", funzionante. FlashGBX 5.1 riconosce il lettore come "GBxCart RW v1.4a/b/c", firmware R42+L14 (2025-05-23); un aggiornamento a R42+L15 (2026-06-03) è disponibile e rimandato per ADR-061, senza urgenza perché è un'operazione sul lettore e non sui dati di gioco. La sequenza della prima sessione sta in `gba-save-extraction-smeraldo/RUNBOOK-PRIMA-SESSIONE.md`. Della sua revisione va conosciuto prima di collegarlo un fatto che nessuna documentazione di prodotto contiene, cioè che l'interfaccia parte alla tensione più alta e che inserire una cartuccia di seconda generazione in quella condizione ne cancella il salvataggio anche senza comando di connessione: il rimedio è una sequenza e sta nel runbook della pila. Il gioco su cui si interviene per il bug dell'inventario gira su Game Boy Advance SP.
...
Il track del trading LDN aggiunge una console Nintendo Switch e un requisito che non ha equivalenti negli altri sottoprogetti: una scheda Wi-Fi capace di modalità monitor. È il fattore critico dichiarato, non un dettaglio, perché le schede USB esterne di fascia adatta risultano affidabili nei test degli autori del proof of concept, mentre alcune schede interne moderne sono fino alla metà più lente e possono andare in deadlock. La domanda su questa macchina è chiusa in due tempi: non ha alcuna interfaccia Wi-Fi integrata, e l'adattatore con cui si parte è l'Archer T2U Nano che l'utente possiede già. Il primo passo non è quindi un acquisto ma una misura, cioè collegarlo e leggerne l'identificatore USB, perché il nome commerciale copre chip diversi e la riserva registrata in precedenza poggiava su una premessa sbagliata: la testimonianza positiva riguardava un modello con chip RTL8821CU servito dal driver in albero, non l'RTL8811AU che si era supposto. Lo stesso lettore GBxCart RW del track Smeraldo compare fra i requisiti, ma il materiale non spiega in quale fase entri.
...
Sul PC, che è una macchina Windows 11, la catena del sottoprogetto Smeraldo è composta dai driver seriali CH340 e CH341, da FlashGBX per leggere e scrivere il salvataggio, dal runtime .NET Desktop e da PKHeX. La scelta di Windows non è una preferenza: PKHeX è un'applicazione .NET Windows Forms e il supporto a Mono e Wine è stato abbandonato dal 2023, quindi le alternative non sono equivalenti. Verificato sui due giri di correzione scritti sulla cartuccia vera il 2026-09-17: PKHeX serve all'utente per l'ispezione (il censimento dei box in CSV che ha aperto il fronte dei glitch storici), non per applicare le correzioni, che sono invece script Python deterministici e riverificabili sotto `gba-save-extraction-smeraldo/tools/`, per lo stesso principio di `token-economy.md` sul preferire il deterministico al manuale quando entrambi sono disponibili. Gli script di scrittura sono cinque al 2026-09-18, uno per famiglia di modifica: `emerald_bag_fix.py` e `emerald_bag_fix_round2.py` sullo zaino e sulla squadra, `emerald_key_item_add.py` sulla sola tasca Oggetti Chiave, `emerald_event_flags_fix.py` sui flag di abilitazione delle isole e `emerald_encounter_flags_fix.py` sui flag degli incontri. I due di lettura sono `emerald_bag_decode.py` e `emerald_event_flags_decode.py`, e nessuno dei sette dipende da PKHeX.

FlashGBX non era installato alla verifica del 2026-09-15: l'ultima versione pubblicata è la 5.1 del 2026-08-29 (il pacchetto Windows con installer, `FlashGBX-5.1_Windows-x64_Setup.zip`, è stato aggiunto il 2026-09-08), scaricata e verificata sul digest SHA-256 dichiarato dall'API di GitHub prima dell'esecuzione, e installata lo stesso giorno in `C:\Users\Utente\AppData\Local\Programs\FlashGBX`, escludendo dal setup il componente driver perché CH340/CH341 erano già presenti via Windows Update.

**PKHeX è invece già installato da tempo**, in `E:\tools\pkhex\PKHeX.exe`, versione 26.8.26.0 (file datato 2026-09-01): questa scheda non lo dichiarava esplicitamente come presente sul disco e la sua assenza è stata assunta per errore in una tornata di questa stessa sessione, mentre il progetto lo usa da circa un mese per validare i lotti generati di esemplari da evento. L'errore ha anche chiuso da sé la domanda sul runtime .NET: l'eseguibile pesa 56,5 MB e non porta accanto alcun `PKHeX.runtimeconfig.json`, la firma di una pubblicazione self-contained che incorpora il proprio runtime .NET nel file singolo. Non dipende quindi dalle versioni di `Microsoft.WindowsDesktop.App` installate sulla macchina (che restano 8.0.31 e 10.0.12, non la 9 che questa scheda dichiarava prima di questa verifica): funziona a prescindere, ed è la ragione per cui ha già funzionato per un mese senza che nessuno se ne accorgesse.
...
## Il codice del progetto al 2026-09-09, che non è più un solo pacchetto

Questa scheda apriva dichiarando che il progetto non avesse uno stack nel senso consueto e che il codice fosse il solo pacchetto del ponte. La prima metà resta vera e la seconda no, quindi la sezione va letta con questo aggiornamento accanto.

Il pacchetto `pokebridge` copre oggi tre generazioni più lo strato del salvataggio: i lettori e scrittori delle strutture di prima, seconda e terza generazione, il modulo dei metodi di generazione degli eventi, e `save3.py`, che apre un contenitore da 128 KiB, sceglie fra i due slot, ricompone i buffer che le quattordici sezioni frammentano, legge e scrive deposito e squadra ricalcolando i checksum, genera un salvataggio sintetico valido e converte fra la forma del file dell'editor e quella della posizione nel deposito, che non sono la stessa. La suite è a duecentosei prove e continua a non dipendere da nulla oltre alla libreria standard, che resta una scelta da mantenere.

Accanto al pacchetto è cresciuta la cartella `tools/`, che alla verifica del 2026-09-09 contiene cinquantadue programmi e non è una raccolta di script di comodo: è l'attuazione della regola sull'economia dei token, per cui il lavoro deterministico sta su codice e l'LLM interviene solo sul salto semantico. Si dividono in cinque famiglie. I generatori, che compongono esemplari per generazione e per classe di provenienza. I censimenti, che leggono le tabelle del verificatore e producono un documento generato con la propria fonte dichiarata. I lettori di fonti, cioè quelli che portano in casa una pagina, un canale, un post con il suo grafo di rinvii o una cartella di calcolo, per livelli invece che per intero. Gli strumenti di igiene tipografica e di Markdown, che sono quelli propagati dal template. E gli strumenti del documento, cioè copertura, bibliografia, mappa delle fonti e verifica del PDF.

Nessuno di essi aggiunge una dipendenza esterna, e il fatto va dichiarato perché sarebbe stato naturale il contrario: la lettura delle cartelle di calcolo non usa una libreria di terze parti ma il fatto che quel formato sia un archivio con dentro XML, e il recupero delle pagine passa dalla libreria standard e da `curl`, che è già sulla macchina. Le dipendenze esterne restano quindi due, cioè `yt-dlp` installato con pip e `git`, più il Python di sistema.

## Gli strumenti di lettura delle fonti, e da dove passano

Alla catena di trascrizione descritta sopra si sono aggiunte tre vie, e vale registrarle qui perché sono la parte dello stack che decide quali fonti il progetto possa leggere. La prima è `tools/fetch-discord.py`, che parla con l'API ufficiale usando un bot account creato nel portale per sviluppatori, con impaginazione, gestione del limite di frequenza e cursore per leggere il solo delta; il suo presidio è che verifica di essere autenticato come bot prima di leggere e si arresta altrimenti. La seconda è l'esportazione con lo strumento della comunità e il token dell'utente, decisa in ADR-019 per i server dove il bot non può essere invitato, con il token mai scritto in un file. La terza è `tools/fetch-reddit.py`, che dal 2026-09-07 non passa più da Reddit ma dall'archivio pubblico Arctic Shift, senza alcuna credenziale, e che attraversa in ampiezza il grafo dei rinvii invece di leggere un post solo.

Va infine registrato che la macchina con GPU in rete locale resta disponibile e resta non impiegata: le due possibilità che questa scheda le attribuisce, cioè la sintesi locale delle fonti lunghe e la ricerca semantica sul corpus, non sono state implementate, e con duecentonovantadue documenti scaricati nel solo corpus della collezione la seconda ha ora un caso d'uso reale che prima non aveva.
...
Il lato di questo progetto è `tools/read-chat-export.py`, che non tocca il token e lavora su un file già prodotto. Converte l'export in Markdown tenendo solo autore, momento, testo, allegati e indicazione delle risposte, e filtra per parola chiave, intervallo di date e lunghezza minima, che è il modo per togliere il rumore delle conversazioni brevi. Riconosce due formati dalla forma del documento e non dall'estensione: il JSON di DiscordChatExporter e il `result.json` dell'export di Telegram Desktop, dove il testo può essere una lista di frammenti formattati e viene ricomposto. L'esito va in `_notes/fonti/discord/`, e da là si legge o si condensa con il modello locale.
