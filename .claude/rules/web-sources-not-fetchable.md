...
La via è stata percorsa da un capo all'altro il 2026-08-31, su un server di prova creato dall'utente per averne l'amministrazione, e funziona: applicazione creata, Message Content Intent abilitato, invito con i due soli permessi, token in `.env`, e poi elenco dei server, elenco dei canali, lettura della cronologia con il testo presente, filtro che riduce cinque messaggi a due, e cursore che alla corsa successiva riferisce che non c'è nulla di nuovo. Il numero dei permessi da mettere nell'URL di invito è 66560, che è la somma di 1024 per vedere il canale e 65536 per leggerne la cronologia, calcolata e verificata sulla documentazione ufficiale e non copiata da un forum.

Due inciampi dell'allestimento vanno registrati perché costano tempo e non sono deducibili dalla documentazione. Il primo è che la soglia oltre la quale gli intent richiedono una revisione è di diecimila utenti e non di cento server: i cento server sono la soglia della verifica formale dell'applicazione, che è un'altra cosa, e confondere le due porta a credere di dover chiedere un'approvazione che non serve. Il secondo è che il servizio, quando riceve un identificativo che non è un numero, risponde quattrocento con una dicitura sul corpo della richiesta non valido che non nomina il campo: è un errore che sembra un problema di permessi e non lo è, e per questo lo strumento verifica gli identificativi in locale prima di inviare la richiesta.
