...
Il calcolo dà $|V| = 0$ al livello delle specie: l'unione dei titoli permanenti copre l'intero insieme delle milleventicinque voci del catalogo nazionale. Al livello delle forme la differenza contiene dodici elementi, dei quali dieci sono forme totemiche di una generazione e due forme di sola battaglia. Va però dichiarato che questa forma del calcolo impiega il contrassegno di presenza e non l'ottenibilità, e ne discende un limite inferiore e non una misura: il capitolo~\ref{cap:enumerazione} riprende la questione, sostituisce la presenza con l'incontro reale chiuso rispetto alle evoluzioni, e mostra che l'esito numerico regge ma che quattro specie non hanno alcun incontro in alcun titolo permanente.
...
\section{Tre assi, e un collezionabile che non è una specie}

L'elenco di riferimento descritto nelle sezioni precedenti era incompleto, e la sua incompletezza non era una dimenticanza ma un errore di modello: contava due assi dove il dominio ne ha tre.

I due assi presenti erano la specie e la forma. Il terzo è l'esemplare da distribuzione, e la ragione per cui non si riduce ai primi due merita di essere enunciata, poiché la riduzione è intuitiva e falsa. Un esemplare distribuito in un evento porta un nome di allenatore, un identificatore e una data che nessun incontro ordinario produce; è dunque un collezionabile distinto anche quando la sua specie è già coperta, e chi possiede l'esemplare ordinario non possiede quello distribuito. La distinzione non è di grado ma di identità: i due oggetti differiscono in campi che nessuna operazione lecita può trasformare l'uno nell'altro.

Ne discende una proprietà dell'elenco che vale registrare come criterio generale. Il numero degli assi di un elenco di riferimento non si sceglie: lo impone il dominio, ed è il numero delle dimensioni lungo le quali due oggetti possono differire in modo che l'uno non sostituisca l'altro. Un elenco con troppi assi è ridondante e si nota subito; uno con troppo pochi è silenziosamente incompleto, e la sua incompletezza si scopre soltanto quando qualcuno chiede di spuntare una voce che l'elenco non contiene.

\subsection*{Le due grandezze, e quale delle due misura il lavoro}

Enumerato il terzo asse, il conteggio produce due numeri che è tentante confondere e che misurano cose diverse.

Le voci da distribuzione soggette alla scadenza sono $3095$ e le specie distinte che esse portano sono $433$, nella misura che segue l'allargamento dell'insieme discusso nel capitolo~\ref{cap:enumerazione}; prima di quell'allargamento le due grandezze valevano $2686$ e $336$, ed erano il conto di due sole fonti su tre. Il rapporto è di circa otto a uno, e la causa è duplice: la medesima distribuzione compare come voce distinta per ogni regione e ogni lingua in cui fu effettuata, e le generazioni distribuirono in larga parte le medesime specie.

Le due grandezze misurano obiettivi diversi. Se l'obiettivo è il catalogo, cioè la casella accesa, il lavoro utile è di $433$ unità e ogni voce oltre la prima per ciascuna specie è ridondante. Se l'obiettivo è la collezione delle distribuzioni nella loro identità storica, il lavoro è di $3095$ unità e nessuna voce è ridondante. Poiché il primo anello della catena di trasferimento muove sei esemplari per sessione, i due obiettivi non differiscono per fatica ma per scala, e la scelta fra essi determina se il fattore limitante sia la produzione o il trasporto. Un progetto che non la faccia esplicitamente la fa comunque, per accumulo, e la scopre quando il tempo è finito.

\subsection*{Il costo non cresce con l'età, e la ragione per cui sembra crescere}

Il dato più utile emerso dall'enumerazione contraddice un'aspettativa forte, ed è il genere di risultato che riordina una pianificazione.

L'aspettativa è che il materiale più antico sia il più costoso da riprodurre, poiché è il meno documentato e il più distante dagli strumenti correnti. La misura dice il contrario. Le due generazioni più antiche non possiedono un meccanismo di distribuzione digitale, e i loro esemplari da evento sono descritti da tabelle di incontro a record di lunghezza fissa con i campi in chiaro, otto e dodici byte rispettivamente: specie, livello, quattro mosse, una restrizione linguistica e un tipo di donatore. In quelle generazioni non esiste alcun valore di personalità e nessun generatore pseudocasuale da ricostruire, poiché natura, sesso e caratteristiche non derivano da un seme. Comporre un esemplare significa quindi scrivere una struttura con campi noti, che è precisamente ciò che l'infrastruttura già verificata di questo lavoro sa fare.

La generazione intermedia, invece, ha richiesto settimane per centosettantatre voci: la sua tabella vive nel codice di un programma, il suo generatore pseudocasuale ha dovuto essere ricostruito e verificato su duecentonove vettori conservati, e la campagna di giudizio esterno ha prodotto otto difetti. Le quattro generazioni successive portano insieme oltre duemila voci soggette alla scadenza e non richiedono la ricostruzione di alcun algoritmo, poiché ciascun dono è un record binario.

La legge che ne emerge è quindi che in questo dominio il costo per voce non cresce con l'età ma con la presenza di un valore di personalità derivato da un seme, e che tale presenza comincia con la terza generazione e termina con essa nella forma difficile. Vale generalizzarla oltre il caso: il costo di riprodurre un artefatto digitale non è funzione della sua età ma della quantità di stato che va ricostruita anziché letta, e le due grandezze sono correlate solo per accidente storico. Ne segue una prescrizione sull'ordine di lavoro, che è l'inverso di quella suggerita dall'intuizione: si comincia dal materiale più antico, non perché sia urgente, ma perché è quasi gratuito.

\subsection*{Una conclusione da declassare, e perché va fatto ad alta voce}

Il capitolo ha stabilito tre volte, con misure indipendenti, che nessuna specie del catalogo è vincolata dalla scadenza. Quella conclusione va declassata da risposta a limite inferiore, e la ragione è un difetto di lettura che era stato dichiarato nel registro delle fonti e non era stato portato nelle conclusioni.

La misura poggia sul contrassegno di presenza delle tabelle delle statistiche di base, cioè sul fatto che una specie esista nei dati di un titolo raggiungibile senza il servizio intermedio. La presenza nei dati non è però l'ottenibilità nel gioco: una specie può essere presente perché il titolo deve saperla rappresentare quando arriva da un trasferimento, e non essere ottenibile in esso. Ne segue che l'insieme delle specie vincolate dalla scadenza è contenuto in ciò che la misura riporta e non coincide con esso, e che zero è un estremo inferiore.

Il difetto non è di conoscenza ma di trasmissione: il limite era scritto, accanto alla fonte, dal giorno in cui la fonte fu registrata. Un limite dichiarato in un registro e non ripetuto nelle conclusioni che ne dipendono ha lo stesso effetto di un limite non dichiarato, e questa è la sua terza manifestazione in questo lavoro, dopo un'intestazione che dichiarava una numerazione diversa da quella dei propri dati e una nota che diceva di verificare ciò che nessuno aveva verificato. La misura corretta richiede le tabelle degli incontri, che hanno un formato per generazione, ed è il lavoro maggiore fra quelli che restano aperti.
\section{Le prime due generazioni, e la fedeltà che non è decidibile}

L'ordine di lavoro stabilito nella sezione precedente prescriveva di cominciare dal materiale più antico, e la sua esecuzione ha prodotto due risultati: uno atteso, cioè che il costo fosse basso, e uno che non lo era e che riguarda i limiti di ciò che si può dimostrare.

\subsection*{Che cosa esiste, e come lo si legge}

Le due generazioni più antiche non possiedono un meccanismo di distribuzione digitale, e i loro esemplari da evento sono descritti da tabelle di incontro: undici voci per la prima, centocinquantasette per la seconda, in record di lunghezza fissa di otto e dodici byte con i campi in chiaro. Il record della prima porta specie, livello, quattro mosse, una restrizione linguistica e un tipo di donatore; quello della seconda aggiunge un luogo di cattura, un contrassegno di cromaticità e un numero di incubazioni, dove un valore diverso da zero dichiara che la voce è un uovo.

Il tipo di donatore è il campo interessante, poiché non è un dettaglio tecnico ma una classificazione storica compressa in un byte. Nella prima generazione distingue gli esemplari premio consegnati da un gioco su console domestica alla cartuccia collegata da quelli distribuiti a manifestazioni itineranti, e in questi ultimi separa il circuito europeo da quello giapponese. Nella seconda distingue tre localizzazioni del medesimo gioco su console domestica, una serie di distribuzioni fatte in un singolo negozio, e un gruppo in cui il donatore è il destinatario stesso.

\subsection*{Un elenco di nomi come documento storico}

Vale segnalare un caso in cui la struttura dei dati conserva più storia di quanta ne dichiari, poiché è un esempio generalizzabile di come si legge una fonte tecnica.

Per gli esemplari distribuiti alle manifestazioni, la verifica dell'autenticità non può poggiare su un identificatore, che non è fissato, e poggia invece sul nome del donatore, che deve appartenere a un elenco chiuso. Quell'elenco è il documento. Per il circuito europeo contiene diciannove nomi, alcuni di personaggi e altri di paesi, e la loro coesistenza dice che la distribuzione non fu un evento unico ma una serie con una postazione per tappa. Per il circuito giapponese l'elenco si divide in tre insiemi, e i commenti della fonte datano ciascuno: sei nomi di personaggi per una fiera del novembre 1997, cinque nomi di città per una manifestazione itinerante fra il dicembre 1997 e il febbraio 1998, un nome di quartiere per una fiera dell'agosto 1999. Tre eventi distinti vivono dentro un solo valore di un byte, e si separano soltanto guardando quali nomi il verificatore accetti.

Un secondo caso mostra la stessa cosa dal lato delle mosse. Fra gli esemplari premio della seconda generazione ce n'è uno di una specie modesta che conosce una mossa che quella specie non può imparare in nessun modo ordinario. Non è un difetto dei dati: è la firma dell'evento, e la sola presenza di quella combinazione in un salvataggio dichiara da dove l'esemplare provenga. Una mossa impossibile, in questo dominio, è un certificato di origine.

\subsection*{Il risultato sulla fedeltà, che è speculare a quello della terza generazione}

Il capitolo sulle distribuzioni ha mostrato che in terza generazione un singolo valore a trentadue bit determina, attraverso un generatore pseudocasuale, sia l'identità dell'esemplare sia i suoi valori individuali; ne seguiva che, dato un esemplare autentico, la ricerca inversa ne ricava il seme e la fedeltà di una ricreazione diventa decidibile per confronto fra due numeri.

Nelle prime due generazioni quel valore non esiste. I valori individuali furono estratti dal gioco al momento della consegna e non derivano da nulla che l'esemplare porti con sé: non c'è seme da ritrovare, perché non c'è stato alcun seme. Ne segue che la fedeltà su quel campo non è decidibile per difetto di informazione, e non per limite degli strumenti: nessuna analisi, per accurata che sia, può stabilire se una certa combinazione di valori individuali fosse quella di un esemplare consegnato trent'anni prima.

L'asimmetria fra le due nozioni si inverte quindi rispetto alla terza generazione, e vale enunciarla come coppia. Là la legittimità era difficile, perché il generatore andava ricostruito e otto difetti sono emersi solo sotto giudizio esterno, e la fedeltà era dimostrabile. Qui la legittimità è banale, perché qualunque combinazione di valori individuali è legale e non c'è nulla da ricostruire, e la fedeltà è impossibile.

La conseguenza pratica per un progetto di ricreazione è che su queste due generazioni i valori individuali vanno dichiarati come una scelta e non presentati come una ricostruzione, e che le sole voci su cui la fedeltà torna verificabile sono quelle in cui la fonte li fissa: gli esemplari delle manifestazioni, dove i valori sono parte dell'evento, e quelli dichiarati cromatici della seconda generazione, dove la cromaticità non è un contrassegno ma una configurazione dei valori individuali e quindi ne vincola quattro su cinque. Su tutto il resto, il documento generato marca il campo come una scelta dichiarata; ed è preferibile una scheda che dichiari una scelta a una che presenti la medesima cifra come un dato.

\subsection*{Due gruppi senza fonte, e un limite di codifica}

Chiude la sezione ciò che è rimasto fuori, poiché un resoconto senza il proprio complemento non è un resoconto.

Quindici voci della seconda generazione sono uova giapponesi che la fonte classifica come consegnate al destinatario, con mosse assegnate a mano e nessuna indicazione di quale evento fossero. La fonte non le nomina, non le data e non le colloca: il documento generato dichiara i tre campi come non documentati anziché riempirli, e la loro identificazione resta un lavoro aperto. È la scelta opposta a quella che renderebbe il documento più bello, ed è la sola che lo lascia correggibile.

Tre voci non si sono potute comporre per un limite di codifica dichiarato: il nome del loro donatore è in caratteri giapponesi, e il progetto possiede la tabella dei caratteri giapponese della terza generazione ma non quella delle prime due. Il limite è del nostro strumento e non del formato, e va registrato come tale.
\section{Il controllo incrociato, e che cosa produce oltre alla conferma}

Le sezioni precedenti hanno costruito la provenienza degli esemplari da distribuzione delle prime due generazioni a partire da una sola fonte, cioè l'implementazione di riferimento che il lavoro impiega anche come verificatore. Sottoporre quella ricostruzione a una seconda fonte indipendente, un'enciclopedia di dominio, ha prodotto quattro esiti di natura diversa, e vale distinguerli perché soltanto il primo è quello che si attende.

\subsection*{La conferma, che è l'esito atteso e il meno informativo}

Su gran parte dei campi le due fonti concordano: le specie, i livelli, gli assortimenti di mosse, i nomi e gli identificatori dei donatori. La concordanza vale quanto costa, ossia poco: due fonti che descrivono il medesimo artefatto storico hanno buone probabilità di derivare l'una dall'altra, e l'accordo non esclude un errore comune.

Vi è però un caso in cui la conferma è pesante, e si riconosce da una proprietà: le due fonti descrivono il medesimo dato in linguaggi che non si traducono l'uno nell'altro. Il byte che nella struttura di un esemplare porta di norma il tasso di cattura della specie assume, negli esemplari premio distribuiti da un gioco su console domestica, uno di due valori fissi. La fonte tecnica lo presenta come un vincolo che il verificatore impone; l'enciclopedia lo presenta come la confezione, di due tipi, in cui il premio veniva consegnato. Nessuna delle due formulazioni si deduce dall'altra, e la loro coincidenza numerica è dunque una conferma che non poteva nascere per trascrizione.

Un secondo caso della medesima specie riguarda un insieme di valori individuali fissati. La fonte tecnica lo dichiara come una sequenza ordinata di sei numeri; l'enciclopedia lo dichiara nominando le statistiche. La coincidenza fra le due letture, unita a un vincolo interno al dato per cui uno dei sei valori si deriva dagli altri, produce tre accordi indipendenti sulla medesima assegnazione. È il grado di certezza più alto che questo lavoro abbia ottenuto su un campo che nessuna specifica documenta.

\subsection*{La correzione, e la forma in cui l'errore si presentava}

Il secondo esito è la correzione di due affermazioni del lavoro, entrambe della medesima specie: una specie identificata a memoria anziché letta dalla tabella dei nomi.

La forma dell'errore merita attenzione perché non è un errore nei dati. In entrambi i casi il numero della specie era corretto nei byte prodotti, e lo era il nome nella prosa che li descriveva a essere falso. Nel caso peggiore dei due, su quel nome era stata costruita un'affermazione di interesse: si era scritto che una certa mossa fosse impossibile per quella specie, il che era vero della specie erroneamente nominata e non di quella reale, dove la medesima mossa era soltanto rara.

Ne discende un avvertimento sulla scrittura di documentazione tecnica generata. Un documento che deriva i propri numeri da una tabella e la propria prosa dalla memoria di chi scrive contiene due classi di affermazioni con affidabilità incompatibili, e il lettore non ha modo di distinguerle: entrambe si presentano nel medesimo carattere. La disciplina che ne segue è che ogni nome che compaia nella prosa provenga dalla medesima tabella da cui provengono i numeri, e che dove ciò non sia possibile la provenienza sia dichiarata.

\subsection*{La congettura risolta, e la divergenza dichiarata}

Il terzo esito è la risoluzione di una congettura. La fonte tecnica ammette quattro nomi di donatore per un gruppo di distribuzioni, e il lavoro aveva scritto che si trattasse di quattro postazioni oppure di quattro periodi, dichiarando l'alternativa anziché scegliere. L'enciclopedia risolve nel primo senso: nel luogo della distribuzione vi erano quattro macchine, e i quattro nomi sono le quattro macchine. Un campo di un byte diventa un luogo fisico, e l'alternativa dichiarata era la forma corretta in cui portare avanti una domanda aperta.

Il quarto esito è la divergenza, ed è quello su cui la disciplina conta di più. In due punti le fonti non concordano: un assortimento di mosse in cui l'enciclopedia elenca un elemento in più, e una distribuzione con valori individuali che la fonte tecnica non contempla. Le divergenze non si mediano e non si tacciono, e in questo lavoro si risolvono con un criterio dichiarato: si segue la fonte tecnica, non per una gerarchia astratta fra i tipi di fonte, ma perché quella fonte è il verificatore che giudicherà gli esemplari prodotti, dunque è la sua nozione di correttezza a determinare se un esemplare sarà accettato.

Il criterio porta con sé il proprio limite e va registrato insieme a esso: se la fonte tecnica venisse corretta nella direzione dell'enciclopedia, gli esemplari prodotti diventerebbero scorretti senza che alcun controllo lo segnali. La divergenza resta perciò scritta nel documento della provenienza, così che quel giorno la si ritrovi anziché riscoprirla.

\section{Un difetto di procedura, alla sua terza occorrenza}

Chiude il capitolo un difetto che non riguarda i dati né il codice, e che va registrato con la medesima cura perché è costato più di entrambi.

Il lavoro mantiene un registro delle fonti, in cui ogni fonte consultata è annotata con ciò che documenta in modo autorevole. In tre occasioni distinte, nell'arco di due giorni, una domanda è stata dichiarata aperta e messa in coda alla verifica mentre la sua risposta era già scritta in quel registro. Nella prima un'affermazione errata è sopravvissuta per alcune ore. Nella seconda è stata aperta una verifica su un dato privo di significato, e si è chiesto a una persona di rileggere su un dispositivo un numero che era annotato in casa. Nella terza, e la più onerosa, un limite di trasferimento è stato dichiarato da verificare mentre il registro lo documentava da giorni; poiché quel limite definiva la scala dell'intera campagna, la pianificazione ha lavorato per giorni su un tetto che il materiale disponibile non imponeva.

La lettura corretta di questi tre episodi non è che serva più attenzione, poiché l'attenzione non ha mai prevenuto nulla di sistematico. La lettura corretta è che un registro delle fonti è un'infrastruttura di consultazione, e un'infrastruttura di consultazione che non venga consultata prima di dichiarare aperta una domanda produce esattamente l'effetto di non esistere, con il costo aggiuntivo di essere mantenuta. Il rimedio è procedurale e verificabile: prima di dichiarare da verificare un fatto esterno, si cerca quel fatto nel registro, e la ricerca è una singola invocazione di uno strumento di ricerca testuale.

Vale notare la simmetria con il difetto della sezione precedente, poiché le due lezioni sono la medesima applicata a due materiali. Là un nome nella prosa proveniva dalla memoria anziché dalla tabella accanto; qui una risposta proveniva dalla memoria anziché dal registro accanto. In entrambi i casi l'informazione corretta era a distanza di un comando, e in entrambi i casi il costo dell'omissione è stato pagato più tardi e da qualcun altro: nel primo dal lettore, che avrebbe creduto a una frase falsa; nel secondo dalla pianificazione, che ha misurato uno spazio più piccolo del reale.

\section{Una fonte in più, e un quasi-incidente che ne misura il metodo}

L'elenco di riferimento descritto nelle sezioni precedenti si aggiorna quando un lotto nuovo raggiunge lo stato di prodotto e giudicato conforme, e l'aggiornamento più recente riguarda gli scambi in gioco della terza generazione, chiusi diciannove su diciannove dal verificatore esterno. Lo strumento che genera l'elenco è stato esteso con una fonte in più, che legge i diciannove file reali del lotto invece di una tabella separata, ricavando la specie nazionale di ciascuno dagli stessi moduli del ponte già impiegati altrove in questo lavoro.

Il numero complessivo dell'elenco non si è mosso: seicentottantacinque specie coperte su milleventicinque, trecentoquaranta senza alcuna fonte, identico allo stato precedente l'aggiunta. Non è un esito nullo ma la conferma che l'aggiunta è corretta: le specie portate dalla fonte nuova erano già coperte da fonti precedenti, e un numero che resta fermo quando si aggiunge una fonte ridondante è precisamente ciò che ci si deve aspettare. Un numero che si fosse mosso avrebbe richiesto una spiegazione prima di essere accettato.

Il primo lancio della rigenerazione, però, quella spiegazione l'ha richiesta davvero, ed è il quasi-incidente che dà a questa sezione il suo secondo motivo di esistere. Il conteggio delle specie coperte è crollato da seicentottantacinque a settantuno, una variazione che nessuna aggiunta di fonte può produrre in senso diminutivo: aggiungere informazione non toglie mai una copertura già accertata. La causa non era nella fonte nuova ma in un parametro omesso nell'invocazione dello strumento, che escludeva la lettura dei salvataggi esterni della raccolta perché il file che ne conserva il censimento non era più presente su disco.

Il punto che vale registrare non è l'errore, che è ordinario, ma il modo in cui è stato riconosciuto prima di essere consegnato come risultato: un numero che cambia senza una ragione di dominio è quasi sempre un difetto di invocazione e non una scoperta, criterio già enunciato altrove in questo lavoro e qui confermato per la quarta volta su un materiale diverso. Il censimento mancante è stato rigenerato e la rigenerazione della checklist ripetuta prima di accettare l'esito, e il numero tornato al proprio valore atteso ne è stato la verifica.
