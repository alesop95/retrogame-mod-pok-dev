...
Sulla tabella intera, centoquattro esemplari prodotti e sessantanove non producibili. Il programma non produce nulla per le voci che non sa fare, e le elenca con la ragione: un generatore che produca qualcosa per ogni voce è peggio di uno che si rifiuti, perché un esemplare sbagliato in mezzo a centosettanta giusti non si trova guardando.

### I limiti che restano, con il loro costo

Ventisei voci si fermano sulla codifica dei caratteri, e la ragione non è una lacuna della nostra estrazione. La tabella che il progetto possiede copre duecentocinquanta byte su duecentocinquantasei, e i sei mancanti sono di controllo: non manca nulla. Manca una tabella diversa, e il fatto che la rende necessaria vale registrarlo perché non è evidente. Nella terza generazione il medesimo byte rende un glifo diverso secondo la lingua del gioco, quindi il nome di un allenatore giapponese non si scrive con la tabella internazionale: due caratteri di quei nomi, il prolungamento e una sillaba, occupano nella tabella internazionale posizioni che portano altri segni. Serve la tabella giapponese, che è una seconda estrazione da fare e non una correzione.

Quarantatre voci si fermano sul metodo di generazione, e si dividono in tre gruppi di costo molto diverso. Quindici voci impiegano metodi che differiscono dal nostro per uno scostamento: undici consumano due estrazioni in più prima di quelle che contano, per scegliere l'indice del dono, e quattro restringono il seme a un intervallo di duecentoquattordici valori invece dei sessantacinquemila. Sono il lavoro più redditizio che resti, perché il codice è il medesimo con un parametro in più. Venticinque voci impiegano il metodo ordinario delle uova, che è un algoritmo diverso e non uno scostamento. Le tre restanti impiegano un generatore differente o una tabella propria, e sono una ciascuna.

Va detto che nessuno di questi limiti blocca la campagna, e la ragione è quella della sezione precedente: la via di produzione in volume per gli eventi non è obbligata a essere la nostra. Il progetto produce ciò che sa produrre e sa dire che cosa non sa, che è esattamente la condizione per decidere se valga la pena scrivere il resto oppure impiegare per quelle voci lo strumento di terzi.
