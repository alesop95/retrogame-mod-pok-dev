...
### L'hardware che l'utente possiede, e il fattore tre sul tetto

Il 2026-09-02 l'utente ha dichiarato di possedere tre cartucce della quarta generazione, cioè Diamante, Perla e Platino, e la dichiarazione cambia il conto della sezione precedente in un modo che vale registrare.

Il limite di frequenza del primo passaggio è registrato sul salvataggio e non sulla console, poiché è il salvataggio a conservare la data dell'ultima migrazione. Se il limite è di sei esemplari al giorno per salvataggio, con tre salvataggi distinti il tetto passa da poco più di mille esemplari a circa tremila nei giorni che restano, e a quel punto anche il terzo insieme, cioè il Pokedex dal marchio di origine antico, entra nel dominio del possibile invece di essere una fantasia.

Ne segue che la misura da fare non è una ma due, e la seconda vale un fattore tre: quanti esemplari muove un passaggio, e se il limite sia per salvataggio o per console. Le due si risolvono nello stesso pomeriggio, perché la seconda si accerta tentando la migrazione su una seconda cartuccia subito dopo averla fatta sulla prima.

Le tre cartucce servono inoltre al secondo insieme e non soltanto come ponte, perché le carte del dono segreto della quarta generazione si riscattano proprio su quei titoli. È una ragione in più per non consumare un salvataggio avanzato: se Platino è avanti nella partita, conviene usarlo come veicolo per il ponte e riservare le altre due al riscatto delle carte, oppure il contrario, ma la decisione va presa sapendo che quei salvataggi hanno due usi e non uno.

Sull'iniettare un salvataggio che abbia già sbloccato il primo passaggio, la regola del progetto è già scritta e si applica senza modifiche: un salvataggio procurato è un veicolo e non un carico, quindi si prendono i contrassegni di avanzamento, si sostituiscono i dati dell'allenatore con i propri, e le scatole si svuotano invece di verificarne il contenuto uno per uno.

Sull'acquisto di una cartuccia di appoggio della terza generazione, l'utente ne ha dichiarato l'intenzione e la decisione è buona per una ragione che eccede il collezionismo: essa separa il lavoro rischioso dalla cosa irripetibile. Gli esemplari ricreati vanno sulla cartuccia di appoggio, e quella che porta i trecentottantasei originali resta intoccata. È il medesimo principio per cui la regola sull'hardware impone il backup prima di ogni scrittura, applicato un passo prima: non proteggere il dato dalla scrittura, ma non scrivere affatto sul supporto che lo contiene.
